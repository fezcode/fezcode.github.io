...
const ConstellationNode = ({ x, y, size = 40, label, color, icon: Icon, onClick, delay = 0 }) => {
  return (
    <motion.div
    ...
    >
      ...
      >
        {Icon ? <Icon size={size * 0.5} color="white" /> : <div className="w-2 h-2 bg-white rounded-full" />}
      </motion.div>
...
const InfoPanel = ({ data, onClose }) => (
   ...
   >
      ...
      <div className="mt-8">
         ...
         <h2 className="text-3xl font-bold mb-2" style={{ color: data.color }}>{data.label}</h2>
         <p className="text-gray-400 text-sm font-mono mb-6 uppercase tracking-widest">{data.type}</p>
