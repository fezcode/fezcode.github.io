...
const AssetConstructorPage = () => {
  ...
  const drawAsset = useCallback(
    (ctx, canvasW, canvasH, exportMode = false) => {
      ...
      if (style === 'frosty') {
        ...
        ctx.lineTo(cx + s, cy);
        // TR
        ctx.moveTo(cx + cw - s, cy);
        ctx.lineTo(cx + cw, cy);
        ctx.lineTo(cx + cw, cy + s);
        // BL
        ctx.moveTo(cx, cy + ch - s);
        ctx.lineTo(cx, cy + ch);
        ctx.lineTo(cx + s, cy + ch);
        // BR
        ctx.moveTo(cx + cw - s, cy + ch);
        ctx.lineTo(cx + cw, cy + ch);
        ctx.lineTo(cx + cw, cy + ch - s);
        ctx.stroke();
      } else if (style === 'cyber') {
    ...
    [style, bgStyle, width, height, radius, opacity, accentColor, baseColor],
  );
  ...
  const handleDownload = () => {
    ...
    link.click();
    addToast({
      title: 'ASSET_RASTERIZED',
      message: 'High-fidelity component saved to storage.',
    });
  };
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
            <div className="space-y-4">
              ...
              <p className="text-xl text-gray-400 max-w-2xl font-light leading-relaxed">
                Component synthesis protocol. Construct high-fidelity UI
                containers with advanced glassmorphism and technical framing.
              </p>
            </div>
              ...
              <div className="mt-12 p-8 border border-white/10 bg-white/[0.01] rounded-sm flex items-start gap-6">
                ...
                <div className="space-y-4">
                  <p className="text-sm font-mono uppercase tracking-[0.2em] leading-relaxed text-gray-500">
                    ASSET_PROTOCOL: High-fidelity rasterization active. Export
                    process generates a 3000x3000px transparent PNG entity.
                  </p>
                </div>
              </div>
            </div>
