...
const SymbolFlowPage = () => {
  ...
  const initParticles = useCallback((width, height) => {
    ...
    particles.current = Array.from({ length: count }, () => ({
      ...
      vx: 0,
      vy: 0,
      symbol: protocol.symbols[Math.floor(Math.random() * protocol.symbols.length)],
  ...
  const animate = useCallback((time) => {
    ...
    particles.current.forEach((p) => {
      ...
      if (p.life <= 0) {
        ...
        p.symbol = protocol.symbols[Math.floor(Math.random() * protocol.symbols.length)];
      }
      ...
      const angle = Math.sin(noise * 10) * Math.PI + Math.cos(noise * 5) * Math.PI;
  ...
  const handleDownload = () => {
    ...
    link.click();
    addToast({ title: 'FLOW_CAPTURED', message: 'Current stream state exported to local storage.' });
  };
            ...
            <div className="border border-white/10 bg-white/[0.02] p-8 rounded-sm space-y-10">
              ...
              <div className="grid grid-cols-1 gap-2">
                {PROTOCOLS.map((p) => (
                  ...
                  >
                    <span>{p.label}</span>
                    <span className="opacity-40">{p.symbols.slice(0, 3).join('')}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="border border-white/10 bg-white/[0.02] p-8 rounded-sm space-y-10">
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  ...
                  <input type="range" min="10" max="200" value={density} onChange={(e) => setDensity(parseInt(e.target.value))} className="w-full accent-emerald-500 h-1 bg-gray-800 rounded-lg appearance-none cursor-pointer" />
                </div>

                <div className="space-y-4">
                  <div className="flex justify-between font-mono text-[9px] uppercase text-gray-600">
                    <span>Noise_Chaos</span>
                    <span className="text-white">{Math.round(chaos * 10000)}</span>
                  </div>
                  <input type="range" min="0.0001" max="0.01" step="0.0001" value={chaos} onChange={(e) => setChaos(parseFloat(e.target.value))} className="w-full accent-emerald-500 h-1 bg-gray-800 rounded-lg appearance-none cursor-pointer" />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <input type="range" min="0.1" max="10" step="0.1" value={velocity} onChange={(e) => setVelocity(parseFloat(e.target.value))} className="w-full accent-emerald-500 h-1 bg-gray-800 rounded-lg appearance-none cursor-pointer" />
                </div>

                <div className="space-y-4">
                  <div className="flex justify-between font-mono text-[9px] uppercase text-gray-600">
                    <span>Ghosting_Trails</span>
                    <span className="text-white">{Math.round((1 - trail) * 100)}%</span>
                  </div>
                  <input type="range" min="0.01" max="0.5" step="0.01" value={trail} onChange={(e) => setTrail(parseFloat(e.target.value))} className="w-full accent-emerald-500 h-1 bg-gray-800 rounded-lg appearance-none cursor-pointer" />
                </div>
              </div>
            </div>
          </div>
