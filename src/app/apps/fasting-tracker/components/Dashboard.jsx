...
const Dashboard = ({ data, onLogMeal, onAdjustFast }) => {
  ...
  const totalCalories = todaysMeals.reduce(
    (acc, m) => acc + (parseInt(m.calories) || 0),
    0,
  );
      ...
      <div className="border border-[#1a1a1a] bg-white/10 p-10 relative overflow-hidden group shadow-sm">
        <div className="flex justify-between items-start mb-12 border-b border-[#1a1a1a]/10 pb-4">
          <h3 className="font-mono text-xs text-[#1a1a1a] uppercase tracking-widest font-bold flex items-center gap-3">
            <TimerIcon weight="bold" size={18} /> Current State:{' '}
            {activeFast ? 'Fasting' : 'Eating'}
          </h3>
        </div>
          ...
          <div className="relative w-72 h-72 flex items-center justify-center mb-12">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] text-[#1a1a1a]/50 uppercase tracking-widest font-bold mt-4">
                {activeFast
                  ? `Goal: ${activeFast.targetLength} Hours`
                  : 'Select a plan below'}
              </span>
            </div>
          </div>
            ...
            <div className="w-full space-y-8">
              ...
              {selectedPlan === 'Custom' && (
                <div className="flex items-center gap-6 border-b-2 border-[#1a1a1a] pb-2 animate-in fade-in slide-in-from-top-2">
                  <label className="text-xs font-mono text-[#1a1a1a] font-bold uppercase">
                    Set Hours:
                  </label>
        ...
        <div className="border border-[#1a1a1a] bg-white/10 p-10 h-full flex flex-col shadow-sm">
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              <div className="flex items-end justify-between mb-4">
                ...
                <div className="text-right">
                  <span className="block text-[10px] text-[#1a1a1a]/40 font-mono uppercase font-bold">
                    Goal
                  </span>

                  <span className="text-lg text-[#1a1a1a] font-mono font-bold">
              ...
              <div className="w-full h-4 border border-[#1a1a1a] bg-[#1a1a1a]/5 p-0.5 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((totalCalories / settings.goalCalories) * 100, 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="mt-16 flex-1 overflow-y-auto max-h-64 pr-4">
              <h4 className="text-[10px] text-[#1a1a1a] font-black uppercase mb-6 tracking-widest border-b border-[#1a1a1a]/5 pb-2">
                Meals Today
              </h4>
              ...
              {todaysMeals.length > 0 ? (
                <div className="divide-y divide-[#1a1a1a]/10">
                  {todaysMeals.map((meal) => (
                    ...
                    >
                      <span className="text-lg text-[#1a1a1a]/80 font-medium group-hover:text-[#1a1a1a] transition-colors">
                        {meal.name}
                      </span>
