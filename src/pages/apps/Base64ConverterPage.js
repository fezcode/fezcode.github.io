...
import { ArrowLeftIcon, CodeIcon, ArrowsLeftRightIcon, CopySimpleIcon } from '@phosphor-icons/react';
...
function Base64ConverterPage() {
  ...
  const encodeBase64 = () => {
    try {
      ...
      addToast({ title: 'Success', message: 'String encoded to Base64.', type: 'success' });
    } catch (error) {
      addToast({ title: 'Encoding Error', message: 'Invalid character set detected.', type: 'error' });
      setOutputText('');
    }
  };
  ...
  const decodeBase64 = () => {
    try {
      ...
      addToast({ title: 'Success', message: 'Base64 sequence decoded.', type: 'success' });
    } catch (error) {
      addToast({ title: 'Decoding Error', message: 'Sequence is not a valid Base64 string.', type: 'error' });
      setOutputText('');
    }
  };
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div>
              ...
              <p className="text-gray-400 font-mono text-sm max-w-md uppercase tracking-widest leading-relaxed">
                Binary-to-text translation layer. Map character sequences to Base64 encoding.
              </p>
            </div>
          </div>
        </header>
