...
const WallpaperEnginePage = () => {
  ...
  const draw = useCallback(() => {
    ...
    if (style === 'bauhaus') {
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(canvas.width - cp, canvas.height - cp - 50);
      ctx.lineTo(canvas.width - cp, canvas.height - cp);
      ctx.lineTo(canvas.width - cp - 50, canvas.height - cp);
      ctx.stroke();
      ...
      ctx.lineTo(vbX + 100, vbY + 180);
      ctx.moveTo(vbX + 100, vbY + 140);
      ctx.lineTo(vbX + 60, vbY + 110);
      ctx.moveTo(vbX + 100, vbY + 140);
      ...
      for (let r = 1; r < 6; r++) {
        ctx.beginPath();
        ctx.arc(centerX, centerY, (canvas.width / 10) * r, 0, Math.PI * 2);
        ctx.stroke();
      }
      for (let a = 0; a < 12; a++) {
      ...
      for (let i = 0; i < count; i++) {
        const x = nextRand() * canvas.width;
        const y = nextRand() * canvas.height;
        const radius = (nextRand() * 200 + 50) * (baseSize / 1000);
        const color = nextRand() > 0.5 ? mainColor : accentColor;
        ctx.save();
        ctx.translate(x, y);
        ctx.strokeStyle = color;
        ctx.lineWidth = 1 + nextRand() * 3;
        ...
        if (nextRand() > 0.6) {
          ctx.globalAlpha = 0.6;
          ctx.fillStyle = color;
          ctx.font = `${Math.floor(10 * (baseSize / 1000))}px monospace`;
          ctx.fillText(`RAD: ${Math.floor(radius)}px`, radius + 5, 0);
          ctx.fillText(`PHI: ${Math.floor(nextRand() * 360)}°`, radius + 5, 12);
          ctx.beginPath();
          ctx.moveTo(0, 0);
          ctx.lineTo(radius, 0);
          ctx.stroke();
      ...
      ctx.beginPath();
      ctx.arc(centerX, centerY, baseSize * 0.37, 0, Math.PI * 2);
      ctx.stroke();
      ctx.setLineDash([]);
    } else if (style === 'pixel') {
      const mainColor = colors[0];
      ...
      for (let x = 0; x < cols; x++) {
        for (let y = 0; y < rows; y++) {
          ...
          if (r > 0.7) {
            ctx.fillStyle = r > 0.9 ? accentColor : mainColor;
            ctx.globalAlpha = 0.2 + nextRand() * 0.5;
            ctx.fillRect(px, py, pixelSize, pixelSize);
          } else if (r > 0.5) {
            ctx.fillStyle = mainColor;
            ctx.globalAlpha = 0.1;
            const sub = pixelSize / 4;
            for (let i = 0; i < 4; i++) {
              for (let j = 0; j < 4; j++) {
                if ((i + j) % 2 === 0) {
            ...
            const s = pixelSize * 0.2;
            ctx.fillRect(px + center - s / 2, py + 2, s, pixelSize - 4);
            ctx.fillRect(px + 2, py + center - s / 2, pixelSize - 4, s);
          }
        }
      }
      ctx.strokeStyle = '#fff';
      ...
      for (let i = 0; i < 3; i++) {
        if (nextRand() > 0.5) {
          const ly = nextRand() * canvas.height;
          ctx.globalAlpha = 0.1;
          ctx.beginPath();
          ctx.moveTo(0, ly);
