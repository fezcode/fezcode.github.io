...
const Sidebar = ({
  ...
  if (!course)
    return (
      <div className="p-8 text-gray-500 text-center font-mono text-[10px] uppercase tracking-widest">
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
        {course.modules.map((module, index) => {
          ...
          return (
            <div
              key={module.id}
              className="group border border-transparent hover:border-white/5 transition-all"
            >
              {' '}
              <button
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <span className="flex-shrink-0 text-[9px] font-bold text-emerald-500/50 w-6 font-mono">
                    [{String(index + 1).padStart(2, '0')}]
                  </span>
                  <span
                    className={`font-bold text-[10px] uppercase tracking-widest truncate ${isModuleComplete ? 'text-gray-500 line-through decoration-gray-700' : 'text-gray-300 group-hover:text-white'}`}
                  >
                      ...
                      return (
                        ...
                        >
                          {isCompleted ? (
                            <CheckCircle
                              size={14}
                              className={`flex-shrink-0 ${isActive ? 'text-emerald-400' : 'text-emerald-500/50'}`}
                              weight="fill"
                            />
                          ) : (
                            <Circle
                              size={14}
                              className={`flex-shrink-0 ${isActive ? 'text-emerald-500' : 'text-gray-700 group-hover/lesson:text-gray-500'}`}
                            />
                          )}
      ...
      <div className="p-4 border-t border-white/10 bg-[#050505] space-y-4">
        ...
        {progressPercentage === 100 ? (
          ...
          <div className="text-center flex flex-col gap-1">
            ...
            <span className="text-[9px] text-emerald-500/30 uppercase tracking-widest">
              Training_Active
            </span>
          </div>
        )}
