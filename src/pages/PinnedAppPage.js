...
const PinnedAppCard = ({ app, index }) => {
  ...
  const accentGradient =
    index === 0
      ? 'from-yellow-400 via-orange-500 to-yellow-600' // Gold
      : index === 1
        ? 'from-gray-200 via-gray-400 to-slate-500' // Silver
        : index === 2
          ? 'from-amber-700 via-orange-800 to-amber-900' // Bronze
  ...
  return (
    <Link to={app.to} className="block group relative h-full">
      ...
      <div
        className={`absolute -inset-[1px] bg-gradient-to-br ${accentGradient} rounded-2xl opacity-30 group-hover:opacity-100 blur-sm transition-opacity duration-500`}
      ></div>
      {/* Main Card Container */}
      <div className="relative flex flex-col h-full bg-gray-900/80 backdrop-blur-xl border border-gray-800 rounded-2xl p-5 overflow-hidden group-hover:border-gray-700 transition-all duration-300 shadow-xl">
        ...
        <div className="absolute -bottom-8 -right-8 text-gray-700 opacity-20 group-hover:opacity-30 group-hover:scale-110 transition-all duration-500 rotate-12 pointer-events-none z-0">
        ...
        <div className="flex justify-between items-start mb-4 relative z-10 border-b border-white/10 pb-3">
          <div className="flex flex-col">
            ...
            <span className="text-xs font-mono text-primary-400/80">
        ...
        <div className="relative z-10 flex-grow">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Cpu
              size={24}
              className="text-gray-800 group-hover:text-gray-600 transition-colors"
            />
          </div>
          <h3 className="text-xl font-bold font-mono text-white mb-2 tracking-tight group-hover:text-primary-400 transition-colors">
            {app.title}
          </h3>
        ...
        <div className="relative z-10 mt-6 pt-3 border-t border-white/10 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-wider text-gray-500 group-hover:text-green-400 transition-colors">
              Online
            </span>
          </div>
          <ArrowRight
    ...
    <div className="min-h-screen bg-gray-950 py-16 sm:py-24 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
        <div className="relative mb-16 text-center">
          ...
          >
            <div className="inline-flex items-center justify-center p-3 mb-4 rounded-full bg-gray-900/50 border border-gray-800 backdrop-blur-sm">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="bg-gray-900/30 border border-gray-800 rounded-3xl h-72 animate-pulse"
              ></div>
            ))}
          </div>
        ) : (
