  ...
  return (
    <section className="border-t border-white/10">
      <div className="max-w-[2400px] mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8 py-24">
        ...
        {/* Content Items - Columns 2-4 */}{' '}
        <div className="md:col-start-2 md:col-span-3 border-l border-white/10 pl-8 flex flex-col">
          {sections.map((section, idx) => (
            <div
              key={idx}
              className="group py-8 border-b border-white/10 last:border-b-0 hover:bg-white/5 -mx-8 px-8 transition-colors"
            >
                ...
                <div className="md:col-span-10 flex flex-col gap-4">
                  ...
                  <div className="text-base font-nunito text-[#e0e0e0] leading-relaxed max-w-3xl">
                    <ReactMarkdown
                      components={{
                        ul: ({ children }) => (
                          <ul className="list-disc pl-4 mb-4 space-y-2 marker:text-white/50">
                            {children}
                          </ul>
                        ),
                        li: ({ children }) => (
