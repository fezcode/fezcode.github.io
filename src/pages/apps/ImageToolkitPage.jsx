...
function Palette({ image }) {
    ...
    getPalette(image, 5, 'hex', { quality: 10 })
      ...
      .catch(() => {
        ...
        addToast({
          ...
          message: 'Failed to map color sequence.',
          type: 'error',
        });
      });
  }, [image, addToast]);
  ...
  if (loading)
    return (
      <div className="flex items-center gap-3 font-mono text-[10px] text-gray-600 uppercase tracking-widest animate-pulse">
      ...
      {palette &&
        palette.map((color, index) => (
          ...
          >
            ...
            <span className="relative z-10 font-mono text-xs font-black uppercase tracking-widest text-white mix-blend-difference opacity-0 group-hover:opacity-100 transition-opacity">
              {color}
            </span>
          </motion.div>
        ))}
      ...
      for (let y = 0; y < height; y += 8) {
        for (let x = 0; x < width; x += 4) {
          ascii +=
            charRamp[
              Math.floor(
                (data[(y * width + x) * 4] / 255) * (charRamp.length - 1),
              )
            ];
        }
        ascii += '\n';
      ...
      <div className="mx-auto max-w-7xl px-6 py-24 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-12">
                ...
                {filterButtons.map((f) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <f.icon
                        size={16}
                        weight={activeEffect === f.id ? 'fill' : 'bold'}
                      />
                      <span>{f.label}</span>
                    </div>
                    {activeEffect === f.id && (
                      <motion.span layoutId="active-tick">●</motion.span>
                    )}
                  </button>
                ))}
              </div>
            ...
            {activeEffect === 'blur' && (
              <div className="bg-white/5 border border-white/10 p-8 rounded-sm animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <div className="relative border border-white/10 bg-white/[0.02] p-8 md:p-12 rounded-sm overflow-hidden group">
              ...
              <div className="relative z-10 flex flex-col gap-12">
                {image ? (
                    ...
                    {activeEffect && activeEffect !== 'palette' && (
                      <div className="space-y-4">
                        <span className="font-mono text-[9px] text-emerald-500 uppercase tracking-widest flex items-center gap-2 font-black">
                          ...
                          Modified Image :: {activeEffect}
                        </span>
                        <div className="border border-emerald-500/20 rounded-sm overflow-hidden bg-black shadow-[0_0_40px_rgba(16,185,129,0.05)]">
                  ...
                  <div className="h-96 border border-white/10 border-dashed rounded-sm flex items-center justify-center text-center">
                ...
                <AnimatePresence>
                  {activeEffect === 'palette' && image && (
                    ...
                    >
                      <h3 className="font-mono text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-8">
                        Color Palette
                      </h3>
                      <Palette image={image} />
                    </motion.div>
                  ...
                  {activeEffect === 'asciiArt' && asciiArtOutput && (
                    ...
                    >
                      <div className="flex justify-between items-center mb-6">
                        <h3 className="font-mono text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                          ASCII Art
                        </h3>
                        <button
                          onClick={() => {
                              ...
                              navigator.clipboard
                                ...
                                .then(
                                  () =>
                                    addToast({
                                      title: 'Success',
                                      message: 'ASCII data copied!',
                                    }),
                                  () =>
                                    addToast({
                                      title: 'Error',
                                      message: 'Failed to copy ASCII data!',
                                      type: 'error',
                                    }),
                                );
                            }
                      ...
                      <pre className="text-[8px] md:text-[10px] text-emerald-400 font-mono bg-black/60 p-8 rounded-sm overflow-x-auto whitespace-pre leading-[0.8]">
                        {asciiArtOutput}
                      </pre>
                    </motion.div>
                  )}
                ...
                {activeEffect &&
                  activeEffect !== 'palette' &&
                  activeEffect !== 'asciiArt' && (
