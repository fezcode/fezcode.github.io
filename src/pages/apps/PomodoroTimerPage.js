        ...
        if (seconds === 0) {
          if (minutes === 0) {
            ...
            if (mode === 'work') {
              ...
              addToast({
                title: 'Pomodoro',
                message: "Work session finished! It's time for a break.",
              });
              ...
              addToast({
                title: 'Pomodoro',
                message: "Break's over! Time to get back to work.",
              });
              selectMode('work');
            }
          } else {
            ...
            <div className="relative z-10 p-4 text-center">
              ...
              <div className="flex justify-center gap-4 mb-6">
                <button
                  ...
                  className={`px-4 py-2 rounded-md font-semibold ${mode === 'work' ? 'bg-app text-white' : 'bg-tb text-app border-app-alpha-50 hover:bg-app/15 border'}`}
                >
                  Pomodoro
                </button>
                <button
                  onClick={() => selectMode('shortBreak')}
                  className={`px-4 py-2 rounded-md font-semibold ${mode === 'shortBreak' ? 'bg-app text-white' : 'bg-tb text-app border-app-alpha-50 hover:bg-app/15 border'}`}
                >
                  Short Break
                </button>
                <button
                  onClick={() => selectMode('longBreak')}
                  className={`px-4 py-2 rounded-md font-semibold ${mode === 'longBreak' ? 'bg-app text-white' : 'bg-tb text-app border-app-alpha-50 hover:bg-app/15 border'}`}
                >
                  Long Break
                </button>
              </div>
              ...
              <div className="flex justify-center gap-6">
                <button
                  onClick={toggleTimer}
                  className="p-4 rounded-full bg-tb text-app border-app-alpha-50 hover:bg-app/15 border"
                >
