...
const Badge = ({
  ...
  <span
    className={`px-2.5 py-1 rounded-full text-[10px] font-medium uppercase tracking-widest border ${color} flex items-center gap-1.5 w-fit shadow-sm font-inter`}
  >
...
const StatCard = ({ title, value, icon: Icon, colorClass, bgClass, delay }) => (
  ...
  >
    ...
    <div
      className={`absolute inset-0 opacity-5 pointer-events-none ${bgClass.replace('bg-', 'bg-')}`}
    />
    ...
    <div className="relative z-10">
      <div className="text-white/60 text-xs font-medium uppercase tracking-widest mb-2 font-inter">
        {title}
      </div>
      <div className="text-4xl font-normal text-white font-inter tracking-tight">
        {value}
      </div>
    </div>
  </motion.div>
...
const DetailCard = ({
  ...
  >
    ...
    <div
      className={`absolute inset-0 opacity-5 pointer-events-none ${bgClass}`}
    />
    ...
    <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-4 relative z-10">
      ...
      <h3 className="text-lg font-playfairDisplay font-medium text-white">
        {title}
      </h3>
    </div>
    <div className="flex-grow font-inter relative z-10">{children}</div>
...
const TableRow = ({
  ...
  >
    <div className="col-span-1 text-white/30 text-xs font-medium font-inter">
      {index}
    </div>
    <div className="col-span-4 flex items-center gap-4">
      ...
      <div>
        <div className="text-sm font-medium text-white font-inter tracking-tight group-hover:text-blue-200 transition-colors line-clamp-1">
          {name}
        </div>
        <div className="text-[11px] text-white/50 uppercase tracking-wider font-normal mt-0.5 font-inter">
          {type}
        </div>
      </div>
    </div>
    ...
    <div className="col-span-2 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0">
      <Link
        to={actionLink}
        className="flex items-center gap-1.5 text-xs font-medium text-white bg-[#1a1a1a] hover:bg-[#252525] px-4 py-2 rounded-full transition-all border border-white/10 hover:border-white/20 font-inter"
      >
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const [postsRes, appsRes, storiesRes, projectsRes, vagueRes] =
          await Promise.all([
          ...
          logCategories.map(async (cat) => {
            try {
              ...
              if (res.ok) {
                ...
                logsData[cat] = {
                  length: (parsed.items || parsed.logs || []).length,
                };
              } else {
  ...
  const totalApps = Object.values(data.apps).reduce(
    (acc, cat) => acc + (cat.apps?.length || 0),
    0,
  );
  const totalProjects = data.projects.length;
  const totalLogs = Object.values(data.logs).reduce(
  ...
  let allApps = [];
  Object.values(data.apps).forEach(
    (cat) => (allApps = [...allApps, ...(cat.apps || [])]),
  );
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-2">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3 pr-8 border-r border-white/5">
              <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-black font-medium font-playfairDisplay">
                F
              </div>
              <span className="text-white font-playfairDisplay font-medium text-lg tracking-tight">
                Fezcodex
              </span>
            </div>
          ...
          <div className="flex items-center gap-4 pb-2 md:pb-0">
            <div className="hidden md:flex items-center gap-2 px-4 py-1.5 bg-[#1a1a1a] rounded-full border border-white/10">
              ...
              <span className="text-xs font-medium text-white/70 tracking-wide font-inter">
                v{version} Stable
              </span>
            </div>
            <Link to="/about/skills">
              <DashboardButton icon={Info} primary>
                About
              </DashboardButton>
            </Link>
          </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="lg:col-span-2 bg-[#080808] border border-white/10 rounded-[32px] p-10 relative overflow-hidden group shadow-2xl flex flex-col justify-between">
            ...
            <div>
              <h1 className="text-3xl font-playfairDisplay text-white mb-2 tracking-tight font-medium">
                System Overview
              </h1>
              <p className="text-white/50 text-sm font-medium font-inter">
                Real-time metrics from the Fezcodex network.
              </p>
            </div>
            <div className="mt-12">
              <div className="text-7xl font-normal text-white tracking-tighter font-inter">
                {totalPosts +
                  totalApps +
                  totalProjects +
                  totalLogs +
              ...
              <div className="text-sm font-medium text-white/50 mt-2 flex items-center gap-2 font-inter">
                <span className="text-emerald-400 flex items-center gap-1">
                  <TrendUp weight="bold" /> Live
                </span>
                <span className="flex items-center gap-1">
                  Total Indexed Items{' '}
                  <Info className="text-white/30" weight="fill" />
                </span>
              </div>
              <div className="text-xs text-white/30 mt-1 font-inter">
                Sum of all Blog Posts, Applications, Projects, Log Entries, and
                Vague Issues currently active in the database.
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#080808] border border-white/10 rounded-[32px] p-8 relative overflow-hidden shadow-2xl flex flex-col justify-center items-center text-center">
            ...
            <h2 className="text-xl font-playfairDisplay text-white mb-2 font-medium">
              Command Center
            </h2>
            <p className="text-white/50 text-xs font-inter mb-6 max-w-[200px] font-normal">
              Access global system settings and developer tools.
            </p>
            <Link to="/commands">
            ...
            <div className="space-y-1">
              {Object.keys(data.apps).map((cat) => (
                <ProgressBar
                  key={cat}
                  label={cat}
                  value={data.apps[cat].apps.length}
                  max={totalApps}
                  color="bg-purple-500"
                />
              ))}
          ...
          >
            <div className="flex gap-4 mb-2">
              <div className="flex-1 bg-[#1a1a1a] p-4 rounded-2xl text-center border border-white/5">
                <div className="text-2xl font-normal text-white font-inter">
                  {projectStatus.active}
                </div>
                <div className="text-[10px] uppercase text-white/50 tracking-widest font-inter">
                  Active
                </div>
              </div>
              <div className="flex-1 bg-[#1a1a1a] p-4 rounded-2xl text-center border border-white/5">
                <div className="text-2xl font-normal text-white/60 font-inter">
        ...
        <div className="bg-[#080808] border border-white/10 rounded-[32px] p-8 shadow-2xl">
          <div className="flex items-center justify-between mb-8 px-2">
            <h3 className="text-xl font-playfairDisplay text-white font-medium">
              Latest Activity
            </h3>
            <Link
              to="/settings"
          ...
          <div className="mt-6 pt-6 border-t border-white/10 flex justify-between items-center px-6">
            <div className="text-xs font-medium text-white/40 tracking-widest font-inter">
              TOTAL NODES LOADED
            </div>
