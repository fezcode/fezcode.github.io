...
function JsonPimlConverterPage() {
  ...
  const handleJsonToPiml = () => {
    ...
    try {
      ...
      addToast({
        ...
        message: 'Data mapped to PIML structure.',
        type: 'success',
      });
    } catch (error) {
      addToast({
        title: 'Parsing Error',
        message: 'Input does not conform to JSON standards.',
        type: 'error',
      });
      setOutput('');
  ...
  const handlePimlToJson = () => {
    ...
    try {
      ...
      addToast({
        title: 'Conversion Success',
        message: 'Data mapped to JSON structure.',
        type: 'success',
      });
    } catch (error) {
      addToast({
        title: 'Parsing Error',
        message: 'Input does not conform to PIML standards.',
        type: 'error',
      });
      setOutput('');
      setErrorMsg(error.toString());
  ...
  const copyToClipboard = () => {
    if (output) {
      ...
      addToast({
        title: 'Copied',
        message: `${selectedFormat} payload stored in clipboard.`,
        duration: 2000,
      });
    }
  };
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/20 p-4 rounded-sm max-w-xs">
              <div className="flex items-start gap-3">
                ...
                <p className="text-[10px] font-mono text-emerald-400 uppercase tracking-wider leading-relaxed">
                  PIML protocol is in active development. Structural parity
                  between JSON and PIML may vary.
                </p>
              </div>
            </div>
          </div>
