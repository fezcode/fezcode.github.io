  ...
  const draw = (e) => {
    ...
    if (lastPoint) {
      ...
      if (speed < 2 && Math.random() > 0.8) {
          ctx.beginPath();
          ctx.arc(currentPoint.x + (Math.random()-0.5)*5, currentPoint.y + (Math.random()-0.5)*5, Math.random()*2, 0, Math.PI*2);
  ...
  return (
    <div className="h-screen w-screen bg-[#f4f1ea] relative overflow-hidden cursor-crosshair select-none">
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-6 items-center bg-white/50 backdrop-blur-md p-4 rounded-full border border-black/5 shadow-2xl pointer-events-auto">
        <button onClick={clearCanvas} className="p-3 text-gray-800 hover:text-red-600 transition-colors" title="Clear Canvas">
        ...
        <button onClick={downloadImage} className="p-3 text-gray-800 hover:text-blue-600 transition-colors" title="Download">
      ...
      <div className="absolute top-6 right-6 text-right pointer-events-none">
          <h2 className="font-playfairDisplay text-3xl text-black/40 italic">Paper & Ink</h2>
          <p className="font-arvo text-sm text-black/30 mt-2">Swift strokes are narrow, slow ones are deep.</p>
      </div>
