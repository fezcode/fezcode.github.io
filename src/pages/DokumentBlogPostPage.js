...
const DokumentBlogPostPage = () => {
  ...
  if (loading) return <div className="min-h-screen bg-[#f3f3f3] flex items-center justify-center font-mono text-black uppercase tracking-widest text-xs animate-progress">Accessing_Restricted_File...</div>;
  if (!post) return null;
  ...
  return (
    <div className="min-h-screen bg-[#f3f3f3] text-[#111] font-sans relative overflow-x-hidden selection:bg-emerald-500/20">
      ...
      <div className="fixed top-0 left-0 w-full h-1 bg-black/5 z-50">
        <div className="h-full bg-emerald-600" style={{ width: `${readingProgress}%` }} />
      </div>

      {/* Hero */}
      <div className="relative h-[50vh] w-full overflow-hidden border-b-2 border-black">
        <GenerativeArt seed={post.attributes.title} className="absolute inset-0 opacity-30 grayscale contrast-125" />
        ...
        <div className="absolute bottom-0 left-0 w-full px-6 pb-12 md:px-12">
            <div className="mb-8 flex items-center gap-4">
                <Link to="/blog" className="inline-flex items-center gap-2 border-2 border-black bg-white px-4 py-1.5 text-xs font-mono font-bold uppercase tracking-widest text-black hover:bg-black hover:text-white transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-16 md:px-12 lg:grid lg:grid-cols-12 lg:gap-24">
        ...
        <div className="lg:col-span-8">
            ...
            >
              <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={{ a: (p) => <MarkdownLink {...p} className="font-bold underline decoration-emerald-600/30 hover:decoration-emerald-600" /> }}>
                {post.body}
              </ReactMarkdown>
            </article>
        </div>
        ...
        <div className="mt-16 lg:col-span-4 lg:mt-0">
            <div className="sticky top-24 space-y-12">
                <div className="border-2 border-black p-8 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden">
                    <div className="absolute -top-4 -right-10 px-12 py-1 bg-emerald-600 text-white font-mono text-[10px] font-bold uppercase rotate-45">AUTHENTICATED</div>
                    ...
                    <div className="space-y-6">
                        <DossierSpec icon={Calendar} label="Date_Logged" value={new Date(post.attributes.date).toLocaleDateString()} />
                        <DossierSpec icon={Clock} label="Archive_Size" value={`${calculateReadingTime(post.body)} MIN_READ`} />
                        <DossierSpec icon={IdentificationBadge} label="Subject_ID" value={slug.substring(0, 8).toUpperCase()} isAccent />
                    </div>

                    <div className="mt-12 flex items-center gap-3 border-t pt-6 border-gray-100">
                        <SealCheck size={32} className="text-emerald-600" weight="fill" />
                        <div className="flex flex-col">
                            <span className="font-mono text-[9px] font-bold uppercase text-gray-400 tracking-widest">Verification_Status</span>
                            <span className="font-mono text-[10px] font-black uppercase text-emerald-700">VERIFIED_ARCHIVE_ENTRY</span>
                        </div>
                    </div>
                </div>
            </div>
      ...
      <ImageModal src={modalImageSrc} alt="Dossier Exhibit" onClose={() => setModalImageSrc(null)} />
    </div>
  );
};
...
const DossierSpec = ({ icon: Icon, label, value, isAccent }) => (
    <div className="flex flex-col gap-1">
        ...
        <span className={`font-mono text-sm font-black uppercase ${isAccent ? 'text-emerald-600' : 'text-black'}`}>
            {value}
        </span>
    </div>
);
