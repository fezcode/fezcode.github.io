...
const ReadingPage = () => {
  const [items, setItems] = useState([]);
    ...
    const filteredItems = items.filter(item => {
        ...
        if (filterStatus === 'UNREAD') return matchesSearch && item.status === 'ACTIVE';
        if (filterStatus === 'READ') return matchesSearch && item.status === 'COMPLETED';
    ...
    useEffect(() => {
        if (selectedItem) {
            ...
            setGlobeColor(selectedItem.status === 'ACTIVE' ? currentTheme.globeActive : currentTheme.globe);
        } else {
    ...
    return (
      <div className={`min-h-screen bg-black ${currentTheme.primary} font-mono ${currentTheme.selection} selection:text-black overflow-hidden relative flex flex-col`}>
        ...
        {/* Top HUD Bar */}
        <header className={`flex-none flex items-center justify-between px-6 py-4 border-b-2 ${currentTheme.borderMain30} relative z-50 bg-black/80 backdrop-blur-md`}>
            <div className="flex items-center gap-4">
               ...
               >
                  {theme === 'matrix' ? <Radioactive weight="bold" /> : <TerminalWindow weight="bold" />}
                  <span>{theme === 'matrix' ? 'PROT: MATRIX' : 'PROT: NEW_VEGAS'}</span>
               </button>
            </div>

            <div className="text-right">
               <h1 className={`text-2xl md:text-3xl font-black tracking-tighter ${currentTheme.primary} uppercase glitch-text`} data-text="MISSION_INTEL">
                  MISSION_INTEL
               </h1>
               <div className={`flex justify-end gap-4 text-[10px] tracking-widest ${currentTheme.accent} mt-1`}>
                  <span className="flex items-center gap-1"><WifiHigh weight="bold" className="animate-pulse" /> NETWORK: SECURE</span>
                  <span className="flex items-center gap-1"><ShieldChevron weight="bold" /> ENCRYPTION: AES-256</span>
               </div>
            </div>
        </header>
        ...
        <div className="flex-grow relative z-10 flex flex-col lg:flex-row overflow-hidden">
            ...
            <div className={`w-full lg:w-5/12 xl:w-4/12 flex flex-col border-r ${currentTheme.borderMain20} bg-black/50 overflow-hidden relative`}>
               <div className={`flex-none flex flex-col px-4 py-3 ${currentTheme.bgMain10} border-b ${currentTheme.borderMain20}`}>
                  <div className="flex justify-between items-center mb-3">
                      <span className={`text-xs uppercase tracking-widest ${currentTheme.textDim600}`}>Feed // {filteredItems.length} Packets</span>
                      <Crosshair className={`${currentTheme.textMain50} spin-slow`} size={20} />
                  </div>

                  {/* Search and Filter Controls */}
                  <div className="flex gap-2">
                      <div className="relative flex-grow">
                          <MagnifyingGlass className={`absolute left-2 top-1/2 -translate-y-1/2 ${currentTheme.textDim600}`} size={14} />
                          <input
                              type="text"
                      ...
                      <div className="relative">
                           <div className={`flex items-center border ${currentTheme.borderMain30} bg-black/50`}>
                              <button
               ...
               <div className="flex-grow overflow-y-auto p-4 space-y-3 pb-24 scrollbar-hide">
                   {loading ? (
                      <div className="space-y-2">
                         {[...Array(6)].map((_, i) => (
                            <div key={i} className={`h-16 ${currentTheme.bgMain05} border-l-2 ${currentTheme.borderMain20} animate-pulse rounded-r-sm`} />
                         ))}
                         <div className={`text-center text-xs animate-pulse mt-4 ${currentTheme.primary}`}>DECRYPTING PACKETS...</div>
                      </div>
                   ) : (
                      filteredItems.length > 0 ? (
                          ...
                          <div className={`text-center mt-10 text-xs ${currentTheme.textDim600} uppercase tracking-widest`}>
                              No Intel Found
                          </div>
                      )
                   )}
          ...
          <div className="hidden lg:flex flex-1 relative bg-black flex-col">
              ...
              <div className="absolute inset-0 z-0">
                  <TacticalGlobe className="w-full h-full opacity-40" accentColor={globeColor} />
                  ...
                  <div className="absolute inset-0 bg-[linear-gradient(rgba(16,185,129,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(16,185,129,0.05)_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none"
                       style={{ backgroundImage: `linear-gradient(${theme === 'matrix' ? 'rgba(16,185,129,0.05)' : 'rgba(245, 158, 11, 0.05)'} 1px, transparent 1px), linear-gradient(90deg, ${theme === 'matrix' ? 'rgba(16,185,129,0.05)' : 'rgba(245, 158, 11, 0.05)'} 1px, transparent 1px)` }}
                  ></div>
              ...
              <div className="relative z-10 flex-grow flex flex-col justify-center p-12 pointer-events-none">
                  {selectedItem ? (
                      ...
                      >
                           {/* Decorative Corners */}
                           <CornersOut className={`absolute top-0 left-0 ${currentTheme.primary} m-2`} weight="bold" />
                           <CornersOut className={`absolute top-0 right-0 ${currentTheme.primary} m-2 rotate-90`} weight="bold" />
                           <CornersOut className={`absolute bottom-0 right-0 ${currentTheme.primary} m-2 rotate-180`} weight="bold" />
                           <CornersOut className={`absolute bottom-0 left-0 ${currentTheme.primary} m-2 -rotate-90`} weight="bold" />
                          ...
                          <div className="flex justify-between items-start mb-6">
                              <div>
                                  <div className="flex items-center gap-3 mb-2">
                                      <span className={`px-2 py-0.5 text-[10px] font-bold tracking-widest border ${
                                          ...
                                          <ScrambleText text={selectedItem.status === 'ACTIVE' ? 'UNREAD' : 'READ'} /> STATUS
                                      </span>
                                      <span className={`text-[10px] ${currentTheme.textDim600} tracking-widest font-mono`}>
                                          ID: <ScrambleText text={`INTEL-${selectedItem.id}-${selectedItem.sector}`} />
                                      </span>
                                  </div>
                              ...
                              <div className={`text-right font-mono text-xs ${currentTheme.textMain60}`}>
                                  <div><MapPin className="inline mr-1"/> <ScrambleText text={`${selectedItem.lat}, ${selectedItem.lng}`} /></div>
                                  <div className="mt-1"><Clock className="inline mr-1"/> {selectedItem.date}</div>
                              </div>
                          </div>

                          <div className={`border-t ${currentTheme.borderMain20} py-6 mb-6`}>
                              <h3 className={`text-[10px] ${currentTheme.accent} uppercase tracking-[0.2em] mb-2 flex items-center gap-2`}>
                                  <FileText /> Mission Briefing
                              </h3>
                              <p className={`${currentTheme.textBody} font-mono text-sm leading-relaxed max-h-40 overflow-y-auto scrollbar-hide`}>
                                  {selectedItem.description}
                              </p>
                          </div>

                          <div className="grid grid-cols-2 gap-4 mb-6">
                              <div className={`${currentTheme.bgMain20} p-3 border ${currentTheme.borderMain10} flex items-center gap-3`}>
                                  <User className={currentTheme.primary.split(' ')[0]} size={24} />
                                  <div>
                                      <div className={`text-[9px] ${currentTheme.accent} uppercase tracking-widest`}>Operative</div>
                                      <div className={`${currentTheme.textBright} text-xs font-bold uppercase`}>{selectedItem.author}</div>
                                  </div>
                              </div>
                              <div className={`${currentTheme.bgMain20} p-3 border ${currentTheme.borderMain10} flex items-center gap-3`}>
                                  <ShareNetwork className={currentTheme.primary.split(' ')[0]} size={24} />
                                  <div className="truncate w-full">
                                      <div className={`text-[9px] ${currentTheme.accent} uppercase tracking-widest`}>Uplink</div>
                                      <div className={`${currentTheme.textBright} text-xs font-bold uppercase truncate`}>{getDomain(selectedItem.link)}</div>
                                  </div>
                              </div>
                          </div>
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-emerald-800 gap-4 pointer-events-none pb-32" style={{ color: theme === 'fallout' ? '#92400e' : '#065f46' }}>
                          <LockKey size={80} weight="duotone" className="animate-pulse opacity-50" />
                          <div className="text-center">
                              <div className="tracking-[0.5em] text-sm font-bold">AWAITING TARGET SELECTION</div>
                              <div className="text-xs mt-2 opacity-60">SELECT INTEL PACKET TO DECRYPT</div>
                          </div>
                      </div>
                  )}
              </div>
      ...
      <style jsx global>{`
        ...
        @keyframes glitch-anim {
          ...
          5% { clip: rect(10px, 9999px, 86px, 0); }
          100% { clip: rect(69px, 9999px, 14px, 0); }
        }
        @keyframes glitch-anim2 {
          0% { clip: rect(21px, 9999px, 7px, 0); }
          5% { clip: rect(96px, 9999px, 90px, 0); }
          100% { clip: rect(8px, 9999px, 4px, 0); }
        }
        @keyframes scanline {
...
const IntelCard = ({ item, isSelected, onClick, getDomain, theme }) => {
   return (
      <motion.div
         ...
         whileHover={{ x: 5, backgroundColor: theme.id === 'matrix' ? 'rgba(16, 185, 129, 0.05)' : 'rgba(245, 158, 11, 0.05)' }}
            ...
            <div className="flex flex-col gap-1 w-full">
               <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                     <span className={`w-2 h-2 rounded-full ${item.status === 'ACTIVE' ? 'bg-red-500 animate-pulse' : theme.primaryBg}`} />
                     <span className={`text-[10px] ${theme.accent} tracking-widest font-mono`}>
                        {item.date}
                     </span>
                  </div>
                  {isSelected && <span className={`text-[9px] ${theme.primaryBg} text-black px-1 font-bold`}>SELECTED</span>}
               </div>

               <h3 className={`font-bold uppercase tracking-wider text-sm leading-tight mt-1 transition-colors truncate ${isSelected ? 'text-white' : `${theme.primary} ${theme.primaryHover.replace('hover:text-', 'group-hover:text-').replace('400', '300')}`}`}>
                  {item.title}
               </h3>

               <div className="flex items-center gap-2 mt-1">
                  <span className={`text-[9px] font-mono ${theme.textDim800} ${theme.textDim600.replace('text-', 'group-hover:text-')} uppercase border ${theme.borderDim30} px-1`}>
                     {getDomain(item.link)}
                  </span>
                  <span className={`text-[9px] font-mono ${theme.textDim800} uppercase`}>
                     {item.clearance}
                  </span>
               </div>
            </div>
