...
const FezynthPage = () => {
  ...
  const playNote = useCallback(
    (freq) => {
      ...
      gainNode.connect(
        isDistorted ? distortionNodeRef.current : analyserRef.current,
      );
  ...
  const handleRecording = () => {
    if (!isRecording) {
      ...
      setIsRecording(true);
      addToast({
        title: 'Recording Protocol Active',
        message: 'Capturing audio buffer...',
        type: 'info',
      });
    } else {
      ...
      addToast({
        ...
        message: 'Sample exported to local storage.',
        type: 'success',
      });
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
          <div className="space-y-4">
            ...
            <p className="text-xl text-gray-400 max-w-2xl font-light leading-relaxed">
              Neural audio synthesis engine. Manipulation of Web Audio
              frequencies via low-level browser protocols.
            </p>
          </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="relative h-64 border-2 border-white/10 bg-black overflow-hidden group">
              ...
              <div className="absolute top-4 right-4 flex items-center gap-2 px-2 py-1 bg-black/80 border border-white/10 rounded-sm">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${isDistorted ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}
                />
