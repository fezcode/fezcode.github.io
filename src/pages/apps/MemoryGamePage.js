...
import { ArrowLeft, Brain, Timer, Target, ArrowsClockwise } from '@phosphor-icons/react';
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-emerald-500/30 pb-32 relative font-sans">
      ...
      <div className="relative h-[40vh] w-full overflow-hidden border-b border-white/10">
        <GenerativeArt seed="Memory Protocol" className="w-full h-full opacity-40 filter brightness-50" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#050505] to-transparent" />

        <div className="absolute bottom-0 left-0 w-full px-6 pb-12 md:px-12">
            <div className="mb-6 flex items-center gap-4">
                <Link to="/apps" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/50 px-4 py-1.5 text-xs font-mono font-bold uppercase tracking-widest text-white backdrop-blur-md transition-colors hover:bg-white hover:text-black">
            ...
            <p className="mt-4 text-gray-400 font-mono text-sm max-w-xl">
               Identification and pattern matching protocol. Secure {CARD_VALUES.length} pairs to complete the sequence.
            </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-16 md:px-12 lg:grid lg:grid-cols-12 lg:gap-24">
        ...
        <div className="lg:col-span-8">
           <div className="memory-game-area relative">
              ...
              {!gameStarted && !gameOver && (
                <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/60 backdrop-blur-sm border border-white/5 p-12 text-center">
                   <div className="max-w-md">
                      <h2 className="text-3xl font-black uppercase tracking-tighter mb-6">Initialise Protocol</h2>
                      <p className="text-gray-400 font-mono text-sm mb-12">Click below to begin the pattern identification sequence.</p>
                      <button
                        onClick={startGame}
                ...
                <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/80 backdrop-blur-md border border-emerald-500/20 p-12 text-center">
                   <div className="max-w-md">
                      <div className="inline-flex p-4 rounded-full bg-emerald-500/10 text-emerald-500 mb-6">
                         <ArrowsClockwise size={48} weight="bold" className="animate-spin-slow" />
                      </div>
                      <h2 className="text-4xl font-black uppercase tracking-tighter mb-2 text-emerald-400">Success</h2>
                      <p className="text-gray-400 font-mono text-xs uppercase tracking-widest mb-12">Total_Moves: {moves} {'//'} Data_Integrity: 100%</p>
                      <button
              ...
              <div className={`cards-grid ${!gameStarted || gameOver ? 'opacity-20 pointer-events-none' : ''}`}>
                {cards.map((card) => (
                  <div
                    key={card.id}
        ...
        <div className="mt-16 lg:col-span-4 lg:mt-0">
            <div className="sticky top-24 space-y-12">
                <div>
                    <h3 className="mb-6 font-mono text-[10px] font-bold uppercase tracking-widest text-gray-500">{'//'} SESSION_METRICS</h3>
                    <div className="space-y-6 border-l border-white/10 pl-6">
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="font-mono text-2xl uppercase text-white font-black">{moves}</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="font-mono text-[9px] uppercase tracking-widest text-gray-500 flex items-center gap-2">
                               <ArrowsClockwise size={12} /> Sequence_Matches
                            </span>
                            <span className="font-mono text-2xl uppercase text-emerald-500 font-black">{matchesFound}<span className="text-gray-700 text-lg">/{CARD_VALUES.length}</span></span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="font-mono text-[9px] uppercase tracking-widest text-gray-500 flex items-center gap-2">
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-sm">
                   <h4 className="font-mono text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-4">Instructions</h4>
                   <ul className="space-y-3 text-[10px] font-mono text-gray-500 uppercase tracking-wider">
                      ...
                      <li>• Align all modules to complete the identification protocol.</li>
                   </ul>
                </div>
            </div>
        </div>
