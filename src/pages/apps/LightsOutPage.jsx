...
const LightsOutPage = () => {
  ...
  const checkWin = useCallback((currentGrid) => {
    return (
      ...
      currentGrid.every((row) => row.every((light) => !light))
    );
  }, []);

  const toggleLight = useCallback(
    (row, col, currentGrid, isInitialSetup = false) => {
      ...
      if (!isInitialSetup) {
        ...
        setMoves((prev) => {
          ...
          if (checkWin(newGrid)) {
            setGameOver(true);
            setMessage(
              `System Shutdown Success :: Dark state achieved in ${next} moves.`,
            );
            addToast({
              message: `Puzzle solved in ${next} moves!`,
              type: 'success',
            });
          }
          return next;
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
            ...
            <div className="flex gap-12 font-mono">
              ...
              <div className="flex flex-col">
                ...
                <span
                  className={`text-3xl font-black ${gameOver ? 'text-emerald-500' : 'text-white'}`}
                >
                ...
                >
                  {grid.map((row, r) =>
                    row.map((lightOn, c) => (
                      ...
                      >
                        {lightOn ? (
                          <LightbulbFilamentIcon size={32} weight="fill" />
                        ) : (
            ...
            <div className="bg-white/5 border border-white/10 p-8 rounded-sm">
              ...
              <ul className="space-y-4 text-xs font-mono text-gray-500 uppercase tracking-wider leading-relaxed">
                <li className="flex gap-3">
                  <span className="text-emerald-500">01</span> Goal: Set all
                  grid nodes to OFF state.
                </li>
                <li className="flex gap-3">
                  <span className="text-emerald-500">02</span> Toggle: Flips
                  self and cardinal neighbors.
                </li>
                <li className="flex gap-3">
                  <span className="text-emerald-500">03</span> Efficiency:
                  Minimise move count for optimal result.
                </li>
              </ul>
            </div>
