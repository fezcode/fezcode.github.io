...
const HudBox = ({
  ...
  <motion.div
    initial={{
      opacity: 0,
      scale: 0.9,
      borderColor: THEMES[theme].hudBorderTransparent,
    }}
  ...
  >
    {/* Corner accents */}
    <div
      className={`absolute top-0 left-0 w-2 h-2 border-t-2 border-l-2 ${THEMES[theme].corner}`}
    />
    <div
      className={`absolute top-0 right-0 w-2 h-2 border-t-2 border-r-2 ${THEMES[theme].corner}`}
    />
    <div
      className={`absolute bottom-0 left-0 w-2 h-2 border-b-2 border-l-2 ${THEMES[theme].corner}`}
    />
    <div
      className={`absolute bottom-0 right-0 w-2 h-2 border-b-2 border-r-2 ${THEMES[theme].corner}`}
    />

    {title && (
      <h3
        className={`text-xs uppercase tracking-[0.2em] ${THEMES[theme].accent} mb-3 flex items-center gap-2`}
      >
        <Aperture className="animate-spin-slow" /> {title}
      </h3>
    )}
    <div className={`${THEMES[theme].textDim} font-mono text-sm`}>
      {children}
    </div>
  </motion.div>
);
...
const MiniTerminal = ({ theme, setTheme }) => {
  ...
  const handleCommand = (cmd) => {
    ...
    const newHistory = [
      ...history,
      {
        type: 'input',
        text: cmd,
        className: `${THEMES[theme].text} font-bold`,
      },
    ];
    ...
    switch (lowerCmd) {
      case 'help':
        ...
        commands.forEach((c) => {
          newHistory.push({
            ...
            content: (
              <span>
                <span className={`${THEMES[theme].text} font-bold`}>
                  {c.cmd.padEnd(10)}
                </span>{' '}
                <span className={THEMES[theme].textDim}>- {c.desc}</span>
              </span>
            ),
          });
        });
      ...
      case 'htop':
          ...
          content: (
            <div className="mb-2">
              <div className="flex gap-2">
                ...
                <span className={THEMES[theme].accent}>
                  [
                  {'|'.repeat(Math.floor(Math.random() * 10) + 5) +
                    '.'.repeat(20)}
                  ]
                </span>
                <span className={THEMES[theme].textDim}>
              ...
              <div className="flex gap-2">
                ...
                <span className={THEMES[theme].textDim}>
                  {memPercent}% ({memUsage}/{totalMem}MB)
                </span>
              </div>
              <div className="flex gap-2">
        ...
        newHistory.push({
          ...
          className: `${THEMES[theme].bg} text-black font-bold`,
        });
        const processes = [{ cmd: 'fez.os', user: 'root' }];
        processes.forEach((p, i) => {
          const pid = Math.floor(Math.random() * 8000 + 1000);
          const cpu = (Math.random() * 5).toFixed(1);
          const mem = (Math.random() * 10).toFixed(1);
          const time = `${Math.floor(Math.random() * 10)}:${Math.floor(
            Math.random() * 60,
          )
            .toString()
          ...
          newHistory.push({
            type: 'output',
            text: row,
            className: i % 2 === 0 ? THEMES[theme].text : THEMES[theme].textDim,
          });
        });
        newHistory.push({
          type: 'output',
          text: `Tasks: ${processes.length} total, 1 running, ${processes.length - 1} sleeping`,
          className: `${THEMES[theme].accent} mt-2`,
        });
        break;
      case 'color':
        ...
        newHistory.push({
          type: 'output',
          text: `DISPLAY MODE CHANGED TO: ${newTheme.toUpperCase()}`,
          className: `${THEMES[newTheme].accent} font-bold`,
        }); // Use newTheme for immediate feedback color
        break;
      case 'traits':
        newHistory.push({
          type: 'output',
        ...
        Object.entries(aboutData.traits).forEach(([key, trait]) => {
          ...
          newHistory.push({
            type: 'output',
            text: `  > ${trait.desc}`,
            className: `${THEMES[theme].textDim} pl-4`,
          });
        });
        break;
      case 'skills':
        ...
        aboutData.skills.forEach((skill) => {
          ...
          newHistory.push({
            ...
            content: (
              <span>
                <span
                  className={`${THEMES[theme].text} font-bold inline-block w-24`}
                >
                  {skill.name}
                </span>
      ...
      case 'exp':
        ...
        aboutData.experience.forEach((exp) => {
          newHistory.push({
            ...
            content: (
              <span>
                <span className={THEMES[theme].textDim}>[{exp.period}]</span>{' '}
                <span className={`${THEMES[theme].text} font-bold`}>
                  {exp.company}
                </span>
              </span>
          ...
          newHistory.push({
            type: 'output',
            text: `  > ${exp.role}`,
            className: `${THEMES[theme].accent} pl-4`,
          });
        });
        break;
      case 'status':
        newHistory.push({
          type: 'output',
          text: 'SYSTEM NORMAL. ALL SYSTEMS GO.',
          className: `${THEMES[theme].text} font-bold`,
        });
        newHistory.push({
          type: 'output',
          text: `UPTIME: ${aboutData.stats.find((s) => s.label === 'Uptime')?.value || '99.9%'}`,
          className: THEMES[theme].textDim,
        });
        break;
      case 'whoami':
        newHistory.push({
          type: 'output',
          text: `USER: ${aboutData.profile.name}`,
          className: `${THEMES[theme].text} font-bold`,
        });
        newHistory.push({
          type: 'output',
          text: `ROLE: ${aboutData.profile.role}`,
          className: THEMES[theme].textDim,
        });
        break;
      case 'contact':
        ...
        contacts.forEach((c) => {
          newHistory.push({
            ...
            content: (
              <span>
                <span className={`${THEMES[theme].text} w-20 inline-block`}>
                  {c.label}:
                </span>{' '}
                <span className={THEMES[theme].textDim}>{c.val}</span>
              </span>
            ),
          });
        });
      ...
      case 'version':
        newHistory.push({
          type: 'output',
          text: `FEZ.OS BUILD v${version}`,
          className: `${THEMES[theme].accent} font-bold`,
        });
        break;
      case 'history':
      ...
      case 'home':
        newHistory.push({
          type: 'output',
          text: 'INITIATING TRANSPORT SEQUENCE...',
          className: 'animate-pulse text-red-500 font-bold',
        });
      ...
      case 'bsod':
        newHistory.push({
          type: 'output',
          text: 'FEZ.OS cannot be crashed. Sorry, not sorry.',
          className: `${THEMES[theme].text} font-bold`,
        });
        break;
      case 'date':
        newHistory.push({
          ...
          className: THEMES[theme].text,
        });
        break;
      case 'motd':
        newHistory.push({
          type: 'output',
          text: 'WELCOME, OPERATOR. SYSTEM STATUS: OPTIMAL. ACCESS GRANTED.',
          className: `${THEMES[theme].accent} `,
        });
        break;
      case 'ping':
        newHistory.push({
      ...
      case 'os':
        ...
        newHistory.push({
          type: 'output',
          text: `  OS Name: FEZ.OS Galactic Edition`,
          className: THEMES[theme].textDim,
        });
        newHistory.push({
          type: 'output',
        ...
        newHistory.push({
          type: 'output',
          text: `  Kernel Version: 3.5.0-2077-generic-cigdem-edition`,
          className: THEMES[theme].textDim,
        });
        newHistory.push({
          type: 'output',
          text: `  Build Date: ${new Date().toISOString().slice(0, 10)}`,
          className: THEMES[theme].textDim,
        });
        newHistory.push({
          type: 'output',
          text: `  Hostname: FEZ-GH_PAGES_SERVER-ALPHA-01`,
          className: THEMES[theme].textDim,
        });
        newHistory.push({
          type: 'output',
          text: `  GUI: FEZ.OS React Shell`,
          className: THEMES[theme].textDim,
        });
        break;
      case 'neofetch':
        const uptimeSeconds =
          Math.floor(Math.random() * (24 * 3600 * 7)) + 3600; // Up to 7 days + 1 hour
      ...
      case 'publish':
        ...
        newHistory.push({
          type: 'output',
          text: '* Usability Study of a Novel Tool: The Virtual Cafeteria in Nutrition Education (2020)',
          className: `${THEMES[theme].textDim} list-disc ml-6`,
        });
        newHistory.push({
          type: 'output',
          text: '* The Virtual Cafeteria: An Immersive Environment for Interactive Food Portion-Size Education (2018)',
          className: `${THEMES[theme].textDim} list-disc ml-6`,
        });
        break;
      default:
  ...
  return (
    <div
      className={`h-[400px] bg-black border ${THEMES[theme].border} p-2 font-mono text-xs overflow-hidden flex flex-col`}
    ...
    >
      <div className="flex-grow overflow-y-auto custom-scrollbar">
        {history.map((line, i) => (
          <div
            key={i}
            className={`whitespace-pre ${line.className || (line.type === 'input' ? `${THEMES[theme].text} mt-1` : THEMES[theme].textDim)}`}
          >
            {line.type === 'input' ? '> ' : ''}
...
const NeuromancerHUD = () => {
  ...
  return (
    ...
    >
          ...
          <div className="flex gap-4 mt-4 md:mt-0">
            {aboutData.stats.map((stat, idx) => (
              <div key={idx} className="text-right">
                <div className={`text-xs ${THEMES[theme].textDim} uppercase`}>
                  {stat.label}
                </div>
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {Object.entries(aboutData.traits).map(([key, trait]) => (
                <div key={key} className="group">
                  <div
                    className={`${THEMES[theme].accent} text-xs mb-1 group-hover:text-white transition-colors uppercase`}
                  >
                    {key}
                  </div>
                  <div
                    className={`font-bold flex items-center gap-2 ${key === 'kryptonite' ? 'text-red-400' : ''}`}
                  >
                    {trait.icon && (
                      <trait.icon
                        className={key === 'superpower' ? 'animate-pulse' : ''}
                      />
                    )}
                    {trait.title}
                  </div>
                  ...
                  <HudBox title="War Story Analysis" theme={theme}>
                    <div className={THEMES[theme].textDim}>
                      <strong className="block text-white mb-2">
                        {aboutData.traits.warStory.title}
                      </strong>
            ...
            {activeTab === 'skills' && (
              ...
              >
                {aboutData.skills.map((skill, i) => (
                  <div
                    key={i}
                    className={`flex items-center gap-4 ${THEMES[theme].bgDim} p-2 border ${THEMES[theme].border}`}
                  >
                    <skill.icon size={24} />
                    <div className="flex-1">
                      ...
                      <div
                        className={`h-2 ${THEMES[theme].bg} w-full overflow-hidden`}
                      >
                        <motion.div
                          initial={{ width: 0 }}
            ...
            {activeTab === 'logs' && (
              ...
              >
                {aboutData.experience.map((exp, i) => (
                  <div
                    key={i}
                    className={`border-l-2 ${THEMES[theme].borderStrong} pl-4 py-2 hover:${THEMES[theme].bgDim} transition-colors`}
                  >
                    <div className="flex justify-between items-baseline mb-1">
                      ...
                      <span
                        className={`text-xs ${THEMES[theme].bg} px-2 py-1 rounded text-black font-bold`}
                      >
                        {exp.period}
                      </span>
                    </div>
                    <div className={`${THEMES[theme].accent} text-sm mb-2`}>
                      {exp.role}
                    </div>
      ...
      {/* Useless Footer */}
      <div
        className={`fixed bottom-2 right-4 text-[10px] ${THEMES[theme].textDim} font-mono opacity-50 select-none z-50 text-right`}
      >
