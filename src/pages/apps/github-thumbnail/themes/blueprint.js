...
export const blueprint = (ctx, width, height, scale, data) => {
  ...
  const values = [
    repoName.substring(0, 20).toUpperCase(),
  ...
  labels.forEach((label, i) => {
    ...
    ctx.fillText(
      values[i],
      blockX + blockW * 0.3 + 10 * scale,
      blockY + i * rowH + rowH / 2,
    );
  });
  ...
  ctx.lineTo(width / 2, height - padding); // Vert
  ctx.moveTo(padding, height / 2);
  ctx.lineTo(width - padding, height / 2); // Horiz
  ctx.stroke();
  ctx.setLineDash([]);
  ...
  // Arrows
  ctx.beginPath();
  ctx.moveTo(width / 2 - textW / 2 + 20 * scale, lineY - 10 * scale);
  ctx.lineTo(width / 2 - textW / 2, lineY);
  ctx.lineTo(width / 2 - textW / 2 + 20 * scale, lineY + 10 * scale);
  ctx.moveTo(width / 2 + textW / 2 - 20 * scale, lineY - 10 * scale);
  ctx.lineTo(width / 2 + textW / 2, lineY);
  ctx.lineTo(width / 2 + textW / 2 - 20 * scale, lineY + 10 * scale);
  ctx.fill();
