...
const SpriteEditorPage = () => {
  ...
  const currentTheme = isWhiteBackground ? themeClasses.light : themeClasses.dark;
        ...
        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_200px] gap-8 mt-8">
          ...
          <div
            ...
            <div className="grid grid-cols-2 gap-2">
              <ToolButton icon={PencilSimple} active={tool === 'pencil'} onClick={() => setTool('pencil')} label="Draw" currentTheme={currentTheme} />
              <ToolButton icon={Eraser} active={tool === 'eraser'} onClick={() => setTool('eraser')} label="Erase" currentTheme={currentTheme} />
              <ToolButton icon={PaintBucket} active={tool === 'fill'} onClick={() => setTool('fill')} label="Fill" currentTheme={currentTheme} />
              <ToolButton icon={GridFour} active={showGrid} onClick={() => setShowGrid(!showGrid)} label="Grid" currentTheme={currentTheme} />
              <ToolButton
            ...
            <div className="mt-8">
              <h3 className={`text-xs uppercase tracking-widest ${currentTheme.panelTitle} mb-4 text-center`}>Actions</h3>
              <div className="grid grid-cols-2 gap-2">
                <ToolButton icon={ArrowCounterClockwise} onClick={undo} disabled={history.length === 0} label="Undo" currentTheme={currentTheme} />
                <ToolButton icon={Trash} onClick={clearCanvas} label="Clear" currentTheme={currentTheme} />
              </div>
            </div>

            <div className="mt-8">
              <h3 className={`text-xs uppercase tracking-widest ${currentTheme.panelTitle} mb-4 text-center`}>Grid Size</h3>
          ...
          <div
            ...
            <div
              ...
              onMouseLeave={() => { /* Stop drawing if dragging out */
              }}
              ...
              {pixels.map((color, i) => (
                <div
                  ...
                  style={{
                    ...
                    border: showGrid ? `1px solid ${currentTheme.canvasGrid}` : 'none',
                    cursor: 'crosshair'
                  }}
                />
              ))}
          ...
          <div
            className={`${currentTheme.panelBg} p-4 rounded-lg border-4 ${currentTheme.panelBorder} ${currentTheme.panelShadow} h-fit`}>
            <h3 className={`text-xs uppercase tracking-widest ${currentTheme.panelTitle} mb-4 text-center`}>Palette</h3>
            ...
            <div className="mt-8 border-t-2 border-[#333] pt-4">
              <h3 className={`text-xs uppercase tracking-widest ${currentTheme.panelTitle} mb-4 text-center`}>Export</h3>
              <div className="flex flex-col gap-2">
                <button onClick={() => exportImage('png', 1)}
...
const ToolButton = ({icon: Icon, active, onClick, disabled, label, currentTheme}) => (
  <button
    onClick={onClick}
    disabled={disabled}
    title={label}
