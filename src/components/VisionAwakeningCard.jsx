...
import { SparkleIcon, AtomIcon } from '@phosphor-icons/react';
...
const VisionAwakeningCard = ({
  ...
  const themeClasses =
    customColors || defaultThemes[variant] || defaultThemes.dark;
  ...
  const patternLines = useMemo(() => {
    ...
    return Array.from({ length: 60 }).map(() =>
      Array.from({ length: 150 })
        .map(
          () => patternChars[Math.floor(Math.random() * patternChars.length)],
        )
        .join(''),
    );
  }, []);
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-[5] pointer-events-none overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-500">
        <motion.div
          animate={{
            x: ['-100%', '100%'],
            y: ['-100%', '100%'],
      ...
      <div className="relative z-10 flex justify-between items-start">
        <div className="w-10 h-10 md:w-16 md:h-16 mb-8">
          <AtomIcon
            size="100%"
            weight="duotone"
            className={isCustomDark ? 'text-blue-300' : 'text-blue-600'}
          />
        </div>
        <div
      ...
      <div className="relative z-10 space-y-3">
        ...
        {description && (
          <p className="font-mono text-[10px] md:text-xs text-current opacity-70 leading-relaxed uppercase tracking-wider">
            {description}
          </p>
        )}
      </div>
        ...
        <div className="flex flex-wrap gap-2 md:gap-3">
          {tags.map((tag, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-base md:text-xl flex-shrink-0 ${themeClasses.pillIcon}`}
                >
                  {getIcon(tag.icon)}
                </span>
                <span className="font-syne font-bold text-[9px] md:text-xs uppercase tracking-wide min-w-0 flex items-center gap-1">
                  {tag.label}
                  {tag.href && (
                    <span className="text-[10px] opacity-40 group-hover/tag:opacity-100 group-hover/tag:translate-x-0.5 transition-all">
                      ↗
                    </span>
                  )}
                </span>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <span className={`flex-shrink-0 ${themeClasses.pillIcon}`}>
                    {getSecIcon(tag.icon)}
                  </span>
                  <span className="break-words">
                    {tag.label}
                    {tag.href && (
                      <span className="ml-2 opacity-40 group-hover/sectag:opacity-100 inline-block group-hover/sectag:translate-x-0.5 transition-all">
                        ↗
                      </span>
                    )}
                  </span>
                ...
                {!tag.href && (
                  <div className="w-1.5 h-1.5 rounded-full bg-current opacity-50 flex-shrink-0 ml-2" />
                )}
              </TagComp>
            );
          })}
      ...
      {/* Footer Section */}
      {showFooter && (
        <div className="relative z-10 mt-auto pt-6 border-t border-current/10 flex flex-col md:flex-row gap-6 md:gap-10 transition-opacity duration-200">
          <p
          ...
          <div className="flex gap-4 md:w-1/3">
            {subNotes.map((note, i) => (
              <p
                key={i}
                className={`text-[8px] font-mono leading-tight ${themeClasses.subNote}`}
              >
                {note}
              </p>
            ))}
