    ...
    <section className="py-24 border-b border-white/10 overflow-hidden">
      <div className="max-w-[2400px] mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="md:col-start-2 md:col-span-3 md:border-l md:border-white/10 md:pl-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 pr-4">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-instr-serif text-white">
                Explore <br />{' '}
                <span className="italic text-white/70">With</span> Us
              </h2>
            </div>
            {/* Navigation Arrows */}
          ...
          <div
            ref={scrollRef}
            className="flex gap-6 overflow-x-auto pb-12 snap-x snap-mandatory -ml-4 px-4 scrollbar-hide"
          >
            {items.map((item, idx) => (
              <a
                key={idx}
              ...
              >
                ...
                <div className="p-6 relative z-10 flex flex-col h-full justify-between">
                  <div>
                    <h3 className="text-2xl font-instr-serif text-white leading-tight mb-2">
                      {item.title}
                    </h3>
                  ...
                  <div className="flex items-center gap-4 text-xs font-mono">
                    {item.stats &&
                      item.stats.split(' ').map((stat, sIdx) => (
                        <span
                          key={sIdx}
                          className={`${stat.startsWith('+') ? 'text-green-400' : stat.startsWith('-') ? 'text-red-400' : 'text-yellow-400'}`}
                        >
                          {stat}
                        </span>
                      ))}
