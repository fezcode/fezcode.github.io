...
const TierForge = () => {
  ...
  const handleDrop = (e, targetId) => {
    ...
    try {
        ...
        addToast({ title: 'Invalid Drop', message: 'Only Tier Forge items can be dropped here.', type: 'error' });
        return;
    }

    if (!data || !data.itemId || !data.sourceId) {
         addToast({ title: 'Invalid Drop', message: 'Unknown item format.', type: 'error' });
         return;
    }
  ...
  const handleClear = () => {
    setConfirmation({
      isOpen: true,
      title: 'CLEAR_DATA',
      message: 'WARNING: This will permanently remove ALL items from the board and pool.',
  ...
  const handleSaveToLocal = () => {
    try {
      ...
      addToast({ title: 'Error', message: 'Failed to save data.', type: 'error' });
    }
  };
  ...
  const handleLoadFromLocal = () => {
    try {
      const saved = localStorage.getItem(LOCAL_STORAGE_KEY);
      if (!saved) {
        addToast({ title: 'No Data', message: 'No saved layout found.', type: 'info' });
        return;
      }
  ...
  const handleExport = async () => {
    ...
    try {
        addToast({ title: 'Processing', message: 'Generating image artifact...' });
        ...
        console.error(err);
        addToast({ title: 'Error', message: 'Failed to capture artifact.', type: 'error' });
    }
  };

  const handleAddItemsToPool = (newItems) => {
      setPoolItems(prev => [...prev, ...newItems]);
      addToast({ title: 'Uploaded', message: `${newItems.length} assets added to pool.` });
  };
