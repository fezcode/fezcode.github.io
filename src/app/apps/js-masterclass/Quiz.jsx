...
const MultipleChoiceQuestion = ({
  ...
  <div className="space-y-4">
    {question.options.map((option, index) => {
      ...
      let icon = (
        <div className="w-4 h-4 border border-white/20 flex items-center justify-center transition-colors group-hover:border-white/50" />
      );
      ...
      if (showResult) {
        if (index === question.answer) {
          ...
          icon = (
            <div className="w-4 h-4 bg-emerald-500 flex items-center justify-center text-black">
          ...
          icon = (
            <div className="w-4 h-4 bg-rose-500 flex items-center justify-center text-black">
      ...
      return (
        ...
        >
          <div className="flex-shrink-0 font-mono text-[10px] text-gray-500">
            0{index + 1}
          </div>
          <div className="flex-shrink-0">{icon}</div>
          <span
            className={`font-mono text-xs uppercase tracking-wider ${showResult && index === question.answer ? 'text-emerald-400' : 'text-gray-300'}`}
          >
            {option}
          </span>
        </button>
  ...
  <div className="space-y-6">
    ...
    {showResult && !isCorrect && (
      <div className="p-4 border border-emerald-500/20 bg-emerald-500/5 text-[10px] font-mono text-emerald-500 uppercase tracking-widest flex items-center gap-3">
        <CheckIcon weight="bold" /> Correct Answer:{' '}
        <span className="text-white bg-black/50 px-2 py-1 rounded-sm">
          {question.answer}
        </span>
      </div>
    )}
    ...
    {showResult && !isCorrect && (
      <div className="p-4 border border-rose-500/20 bg-rose-500/5 mt-4 space-y-2">
        ...
        <div className="text-[10px] font-mono text-gray-400">
          The code did not satisfy the test case requirements. Ensure you are
          returning the expected value or performing the required action.
        </div>
      </div>
    )}
  </div>
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-2 gap-8">
        ...
        <div className="space-y-2">
          ...
          {rightSideIndices.map((originalIndex) => {
            ...
            const isMatchedBy = Object.keys(matches).find(
              (key) => matches[key] === originalIndex,
            );
            ...
            if (showResult) {
              if (
                isMatchedBy !== undefined &&
                parseInt(isMatchedBy, 10) === originalIndex
              ) {
                borderClass =
            ...
            return (
              <button
                ...
                disabled={
                  showResult ||
                  (activeLeft === null && isMatchedBy === undefined)
                }
      ...
      {showResult && (
        <div className="p-6 border border-white/10 bg-white/5">
          <h5 className="text-[10px] font-mono text-emerald-500 uppercase tracking-widest mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {question.pairs.map((pair, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 text-xs font-mono text-gray-400 border border-white/5 p-2 bg-black/20"
              >
...
const Quiz = ({ questions, onComplete }) => {
      ...
      case 'code-writing':
        try {
          ...
          const testFn = new Function(
            codeAnswer + '; return ' + currentQuestion.testCase,
          );
          correct = testFn() === true;
        } catch (e) {
          console.error(e);
    ...
    return (
      <div className="bg-[#050505] border border-emerald-500/30 p-12 text-center relative overflow-hidden group">
        ...
        <p className="text-gray-400 font-mono text-xs uppercase tracking-wider mb-8 max-w-md mx-auto leading-relaxed">
          Neural pathways established. Concept integration verified at 100%
          efficiency.
        </p>
  ...
  const getIconForType = () => {
    switch (questionType) {
      ...
      case 'code-writing':
        return (
          <CodeIcon size={16} weight="fill" className="text-emerald-500" />
        );
      case 'fill-in-the-blanks':
        return (
          <TextTIcon size={16} weight="fill" className="text-emerald-500" />
        );
      default:
        return (
          <WarningIcon size={16} weight="fill" className="text-emerald-500" />
        );
    }
  };
