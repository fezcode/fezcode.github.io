...
export const useFastingData = () => {
  ...
  const [customFoods, setCustomFoods] = usePersistentState(
    'fasting_tracker_custom_foods',
    [],
  );
  const [settings, setSettings] = usePersistentState(
    'fasting_tracker_settings',
    {
      targetFastLength: 16,
      goalCalories: 2000,
    },
  ...
  const endFast = (endTime = Date.now()) => {
    if (!activeFast) return;
    setFasts(
      fasts.map((f) => (f.id === activeFast.id ? { ...f, end: endTime } : f)),
    );
  };
  ...
  const addMeal = (meal) => {
    setMeals([
      ...meals,
      { ...meal, id: generateId(), timestamp: meal.timestamp || Date.now() },
    ]);
  };
