...
const TheVaguePage = () => {
  ...
  return (
    <div
      className={`vague-editorial ${isInvert ? 'is-invert bg-[#1a1a1a] text-[#f4f4f4]' : 'bg-[#f4f4f4] text-[#1a1a1a]'} ${isNavOpen ? 'has-nav-open' : ''} is-loaded font-instr-serif transition-colors duration-500 min-h-screen`}
    >
      <Seo
      ...
      <header className="c-header md:!left-[4rem]">
        <div className="c-header_inner md:!left-[4rem]">
          <p className="c-header_heading font-instr-sans">
            <Link className="c-header_link" to="/">
              Fezcodex
            </Link>{' '}
            + <span className="c-header_link">The Vague</span>
          </p>
          <h1 className="c-header_name font-instr-serif">
            <a className="c-header_link" href="#introduction">
        ...
        <div className="c-header_buttons font-instr-sans">
          ...
          <button
            className="c-button -whiteInvert"
            onClick={() => setShowTerminal(true)}
          >
            <span className="c-button_label uppercase tracking-widest text-[10px] font-black">
              Terminal View
            </span>
          </button>
        </div>
      ...
      <div>
        <div
          className={`c-nav-bg ${isNavOpen ? 'is-visible' : ''}`}
          onClick={toggleNav}
        ></div>
        <nav
          className={`c-nav border-r ${isInvert ? 'border-[#f4f4f4]/25' : 'border-[#1a1a1a]/25'} bg-inherit`}
        >
            ...
            <div className="c-nav_panel_footer font-instr-sans">
              <div className="c-nav_panel_links -bold">
                <Link className="c-nav_panel_link" to="/">
                  ←
                  <span className="c-nav_panel_label -under">
                    Back to Fezcodex
                  </span>
                </Link>
              </div>
        ...
        >
          <span className="c-navButton_wrap">
            ...
            <span className="c-nav_label font-instr-sans uppercase tracking-widest text-[10px] font-black">
              Table of contents
            </span>
          </span>
        </button>
          ...
          <div className="c-newspaper">
            <div className="c-newspaper_container !pb-0">
              {latestIssue && (
                <div
                  id="introduction"
                  className={`border-b ${isInvert ? 'border-[#f4f4f4]/25' : 'border-[#1a1a1a]/25'}`}
                >
                  <div className="o-container pt-24 md:pt-32 pb-20">
                    ...
                    <header className="c-newspaper_header">
                      <p className="c-newspaper_header_subtitle font-instr-sans uppercase tracking-[0.2em] text-[11px] opacity-50">
                        Issue No. {issues.length} {'//'} {latestIssue.date}
                      </p>
                    ...
                    <div className="mt-[-4rem] font-instr-sans text-center">
                      <button
                        className="c-button -whiteInvert group"
                        onClick={() => handleIssueClick(latestIssue)}
                      >
                        <span className="c-button_label uppercase tracking-widest text-xs font-black">
                          View Publication
                        </span>
                      </button>
                    </div>
              ...
              >
                <div className="o-container py-20">
                  <div className="c-newspaper_columns font-instr-serif text-lg md:text-xl">
                    <p className="mb-6">
                      The Vague is not merely a collection of documents; it is
                      an ongoing exploration into the intersections of digital
                      permanence and ephemeral thought. Each issue represents a
                      localized state of consciousness, captured and formatted
                      for the void. We navigate the silence between the bits,
                      seeking meaning in the static.
                    </p>
                    <p className="mb-6 italic">
                      "In the garden of forking paths, every decision is a
                      digital signature."
                    </p>
                    <p className="mb-6">
                      As we progress through the various volumes, the objective
                      remains constant: to document the evolution of the
                      Fezcodex ecosystem. This editorial space serves as a
                      bridge between technical implementation and philosophical
                      inquiry, providing a platform for long-form reflections
                      that transcend the limitations of traditional commits.
                    </p>
                    <p>
                      Through these pages, we invite you to look closer at the
                      architecture of the unknown. The vague is where clarity
                      begins—a threshold of potential where every whisper
                      carries the weight of a world yet to be built.
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="relative z-10 w-full  pb-px !mb-0">
                <div
                  className={`w-full pt-16 pb-16 px-8 md:px-16 border-b ${isInvert ? 'border-[#f4f4f4]/25' : 'border-[#1a1a1a]/25'}`}
                >
                  <h2 className="font-instr-sans uppercase tracking-[0.3em] text-[11px] opacity-70 font-black">
                    Issue Collection
                  </h2>
                </div>
                <div
                  className={`grid grid-cols-1 md:grid-cols-3 w-full border-l ${isInvert ? 'border-[#f4f4f4]/25' : 'border-[#1a1a1a]/25'}`}
                >
                  {sortedIssues.map((issue, idx) => (
                    <button
                      key={issue.id || idx}
                    ...
                    >
                      <div className="w-full">
                        <span className="font-instr-sans text-[10px] opacity-50 block mb-8 uppercase tracking-widest font-black">
                          No. 0{issues.length - idx}
                        </span>
                      ...
                      <div className="flex flex-col w-full">
                        <div
                          className={`w-16 h-px mb-10 transition-colors ${isInvert ? 'bg-[#f4f4f4]/25 group-hover:bg-[#1a1a1a]/25' : 'bg-[#1a1a1a]/25 group-hover:bg-[#f4f4f4]/25'}`}
                        />
                        <span className="font-instr-sans text-[12px] uppercase tracking-[0.3em] font-black opacity-90">
                          {issue.date}
                        </span>
