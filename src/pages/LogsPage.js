...
import {
  ArrowLeftIcon, LogIcon, MagnifyingGlassIcon, FunnelIcon, XCircle, InfoIcon, StarIcon, SidebarIcon,
...
const categories = ['Book', 'Movie', 'Game', 'Article', 'Music', 'Series', 'Food', 'Websites', 'Tools',];

const iconColors = ['text-red-500', 'text-orange-500', 'text-amber-500', 'text-yellow-500', 'text-lime-500', 'text-green-500', 'text-emerald-500', 'text-teal-500', 'text-cyan-500', 'text-sky-500', 'text-blue-500', 'text-indigo-500', 'text-violet-500',];
...
const LogsPage = () => {
      ...
      try {
        const fetchPromises = categories.map(async (category) => {
          ...
          if (!response.ok) {
            ...
            console.warn(`Category PIML not found for ${category}: ${response.statusText}`,);
            return [];
          }
        ...
        const logsWithId = combinedLogs
          .map((log, index) => ({
            ...log, id: `${log.title}-${log.date}-${index}`, originalIndex: index,
          }))
  ...
  useEffect(() => {
    ...
    setFilteredLogs(logs.filter((log) => {
      ...
      const matchesSearch = log.title.toLowerCase().includes(lowerQuery) || (log.author && log.author.toLowerCase().includes(lowerQuery)) || (log.description && log.description.toLowerCase().includes(lowerQuery));
  ...
  return (<div className="py-16 sm:py-24 bg-gray-950 min-h-screen">
    <div className="mx-auto max-w-7xl px-6 lg:px-8 text-gray-300">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-10">
        ...
        {categories.map((category) => {
          ...
          return (<button
            ...
            style={isSelected ? {
              borderColor: categoryColor, color: categoryColor, boxShadow: `0 0 10px ${categoryColor}33`,
            } : {}}
        ...
        {(selectedCategories.length > 0 || searchQuery) && (<button
      ...
      {loading ? (// Skeleton Grid
        ...
        >
          <AnimatePresence>
            {filteredLogs.map((log) => (<motion.div
              layout
              key={log.id}
      ...
      {!loading && filteredLogs.length === 0 && (<div className="text-center py-20">
        ...
        <div>
          ...
          <button
            onClick={() => {
              ...
              openSidePanel('Rating System Details', <RatingSystemDetail/>);
            }}
