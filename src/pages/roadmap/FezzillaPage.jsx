  ...
  return (
    <div onClick={() => onClick(issue.id)} className="block w-full text-left">
      ...
      >
        {/* Icon / ID */}
        <div
          className={`flex items-center gap-3 ${viewMode === 'board' ? 'mb-2 justify-between' : 'w-24 shrink-0'}`}
        >
          <div className="flex items-center gap-2">
            <StatusIcon status={issue.status} className="w-4 h-4 shrink-0" />
            <span className="font-mono text-[10px] text-gray-500 font-medium tracking-wide group-hover:text-gray-400 transition-colors">
              {issue.id}
            </span>
          </div>
        ...
        <div className={`${viewMode === 'board' ? 'mb-3' : 'grow min-w-0'}`}>
          <h3
            className={`text-[13px] font-medium text-gray-200 truncate group-hover:text-white transition-colors`}
          >
            {issue.title}
          </h3>
        </div>
        ...
        <div
          className={`flex items-center gap-4 ${viewMode === 'board' ? 'justify-between text-[10px]' : 'text-[11px] shrink-0'}`}
        >
          {/* Tags */}
          {issue.category && (
...
const FezzillaPage = () => {
    ...
    const fetchRoadmap = async () => {
      try {
        ...
        if (pimlResponse.ok) {
            ...
            items.forEach((item) => {
              if (item.issues) {
                result = result.concat(
                  flatten(
                    Array.isArray(item.issues) ? item.issues : [item.issues],
                  ),
                );
              } else {
                result.push(item);
          ...
          const flatIssues = flatten(
            Array.isArray(parsed.issues) ? parsed.issues : [parsed.issues],
          );
  ...
  const groupOrder = useMemo(
    () => ['In Progress', 'Planned', 'Backlog', 'Completed', 'Cancelled'],
    [],
  );
  ...
  const groupedIssues = useMemo(() => {
    let filtered = issuesData.filter(
      (i) =>
    ...
    if (statusFilter !== 'All') {
      filtered = filtered.filter((i) => {
        ...
        if (statusFilter === 'Planned')
          return s.includes('planned') || s.includes('todo');
        if (statusFilter === 'Completed')
          return s.includes('complet') || s.includes('done');
        if (statusFilter === 'Backlog')
          return s.includes('backlog') || s.includes('hold');
    ...
    filtered.forEach((issue) => {
      ...
      else if (s.includes('planned') || s.includes('todo'))
        groups['Planned'].push(issue);
      else if (s.includes('complet') || s.includes('done'))
          ...
          <div className="flex gap-6 min-w-max pb-12">
            {groupOrder.map((group) => {
              ...
              return (
                <div
                  key={group}
                  className={`shrink-0 transition-all duration-300 ${isCollapsed ? 'w-[50px]' : 'w-[300px]'}`}
                >
                  {isCollapsed ? (
                    <div
                    ...
                    >
                      <div
                        className="writing-mode-vertical text-xs font-mono font-medium text-gray-500 tracking-widest uppercase rotate-180"
