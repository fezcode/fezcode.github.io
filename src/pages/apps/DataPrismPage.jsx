...
const BoxPlotVis = ({ data, params }) => {
  ...
  return (
    ...
    >
      {data.map((item, i) => {
        const x =
          400 + (i - (data.length - 1) / 2) * (600 / (data.length || 1));
            ...
            {params.showValues && (
              ...
              >
                ...
                <text
                  ...
                  y={yMed}
                  dominantBaseline="middle"
                  style={{ fill: item.color, fontWeight: 'bold' }}
                >
                  {item.median}
                </text>
                <text
...
const DotPlotVis = ({ data, params }) => {
  const points = useMemo(() => {
    return data.flatMap((item, idx) => {
      ...
      const centerX =
        400 + (idx - (data.length - 1) / 2) * (600 / (data.length || 1));
...
const LineGraphVis = ({ data, params }) => {
  ...
  const areaD =
    points.length > 0
      ? `${pathD} L ${points[points.length - 1].x},320 L ${points[0].x},320 Z`
      : '';
    ...
    >
      <defs>
        <linearGradient id="lineGrad" x1="0" y1="0" x2="0" y2="1">
          <stop
            offset="0%"
            stopColor={params.metricAreaColor || '#ffffff'}
            stopOpacity="0.2"
          />
          <stop
            offset="100%"
      ...
      {points.length > 1 && (
        <motion.path
          ...
          d={pathD}
          fill="none"
          stroke={params.metricLineColor || '#ffffff'}
          strokeWidth="2"
          opacity="0.3"
        />
      )}
...
const DataPrismPage = () => {
  ...
  const updateNodeField = (index, field, value) => {
    const newData = [...data];
    newData[index][field] =
      field === 'label' || field === 'color' ? value : parseFloat(value) || 0;
    setData(newData);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#f4f4f4] flex flex-col font-sans selection:bg-emerald-500/30 overflow-hidden">
      <Seo
        title="Data Prism | Fezcodex"
        description="Statistical visualization laboratory."
      />
      ...
      <header className="p-6 border-b border-white/10 flex justify-between items-center bg-[#080808] z-30">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`p-3 border rounded-sm transition-colors ${isPanelOpen ? 'bg-emerald-500 text-black border-emerald-500' : 'border-white/10 text-gray-500 hover:text-white'}`}
          >
            <DatabaseIcon size={20} weight="bold" />
          </button>
          <button
            onClick={handleExportSVG}
            className="flex items-center gap-2 px-4 py-2 bg-white text-black font-black uppercase tracking-widest text-[10px] hover:bg-emerald-400 transition-colors"
          >
        ...
        <AnimatePresence>
          {isPanelOpen && (
            <motion.aside
              ...
              animate={{ x: 0 }}
              exit={{ x: -320 }}
              className="w-80 border-r border-white/10 bg-[#080808] flex flex-col z-20 overflow-hidden"
            >
              <div className="p-6 space-y-8 overflow-y-auto no-scrollbar pb-2 flex-1">
                <div className="space-y-4">
                  <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.3em] flex items-center gap-2">
                    <LayoutIcon size={14} /> Visualization_Type
                  </h3>
                  <CustomDropdown
                    variant="brutalist"
                    fullWidth
                ...
                <div className="space-y-6 pt-6 border-t border-white/5">
                  <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.3em] flex items-center gap-2">
                    <SlidersIcon size={14} /> Visual_Params
                  </h3>
                  {chartType === 'box' && (
                    <div className="space-y-4">
                      <CustomSlider
                        label="Box Width"
                        value={visualParams.boxWidth}
                        onChange={(v) =>
                          setVisualParams({ ...visualParams, boxWidth: v })
                        }
                        min={10}
                        max={100}
                      />
                      <CustomToggle
                        label="Show Values"
                        checked={visualParams.showValues}
                        onChange={() =>
                          setVisualParams({
                            ...visualParams,
                            showValues: !visualParams.showValues,
                          })
                        }
                      />
                    </div>
                  ...
                  {chartType === 'dot' && (
                    <CustomSlider
                      ...
                      value={visualParams.density}
                      onChange={(v) =>
                        setVisualParams({ ...visualParams, density: v })
                      }
                      min={2}
                      max={20}
                    />
                  ...
                  {chartType === 'line' && (
                    <div className="space-y-4">
                      <CustomToggle
                        label="Fill Area"
                        checked={visualParams.showArea}
                        onChange={() =>
                          setVisualParams({
                            ...visualParams,
                            showArea: !visualParams.showArea,
                          })
                        }
                      />
                      <CustomColorPicker
                        variant="brutalist"
                        label="Line Color"
                        value={visualParams.metricLineColor}
                        onChange={(val) =>
                          setVisualParams({
                            ...visualParams,
                            metricLineColor: val,
                      ...
                      <CustomColorPicker
                        variant="brutalist"
                        label="Area Color"
                        value={visualParams.metricAreaColor}
                        onChange={(val) =>
                          setVisualParams({
                            ...visualParams,
                            metricAreaColor: val,
                  ...
                  {chartType === 'pie' && (
                    <CustomToggle
                      ...
                      checked={visualParams.showLabels}
                      onChange={() =>
                        setVisualParams({
                          ...visualParams,
                          showLabels: !visualParams.showLabels,
                        })
                      }
                    />
                  )}
                ...
                <div className="space-y-4 pt-6 border-t border-white/5 flex flex-col">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.3em] flex items-center gap-2">
                      <TrendUpIcon size={14} /> Dataset_Nodes
                    </h3>
                    <button
                  ...
                  <div className="space-y-4 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar">
                    {data.map((item, i) => (
                      <div
                        key={i}
                        className="p-4 bg-white/5 border border-white/5 space-y-4 group rounded-sm"
                      >
                        <div className="flex justify-between items-center">
                          <input
                            value={item.label}
                            onChange={(e) =>
                              updateNodeField(i, 'label', e.target.value)
                            }
                            className="bg-transparent border-none text-[10px] font-mono text-white focus:ring-0 uppercase tracking-widest w-full font-bold"
                          />
                          <button
                            onClick={() => handleRemoveNode(i)}
                            className="text-red-500 opacity-30 group-hover:opacity-100 transition-opacity"
                          >
                            <TrashIcon size={14} />
                          </button>
                        </div>
                        ...
                        {chartType === 'box' ? (
                          <div className="grid grid-cols-2 gap-2">
                            <MetricInput
                              label="MIN"
                              value={item.min}
                              onChange={(v) => updateNodeField(i, 'min', v)}
                            />
                            <MetricInput
                              label="MAX"
                              value={item.max}
                              onChange={(v) => updateNodeField(i, 'max', v)}
                            />
                            <MetricInput
                              label="Q1"
                              value={item.q1}
                              onChange={(v) => updateNodeField(i, 'q1', v)}
                            />
                            <MetricInput
                              label="Q3"
                              value={item.q3}
                              onChange={(v) => updateNodeField(i, 'q3', v)}
                            />
                          ...
                          <MetricInput
                            label="VALUE"
                            value={item.value}
                            onChange={(v) => updateNodeField(i, 'value', v)}
                          />
                        )}
                        <CustomColorPicker
                          variant="brutalist"
                          label="Node Color"
                          value={item.color}
                          onChange={(val) => updateNodeField(i, 'color', val)}
                        />
                      </div>
                    ))}
                  </div>
        ...
        <main className="flex-1 relative bg-black flex items-center justify-center p-8 md:p-20">
          ...
          >
            <AnimatePresence mode="wait">
              ...
              >
                {chartType === 'box' && (
                  <BoxPlotVis data={data} params={visualParams} />
                )}
                {chartType === 'dot' && (
                  <DotPlotVis data={data} params={visualParams} />
                )}
                {chartType === 'bar' && <BarChartVis data={data} />}
                {chartType === 'line' && (
                  <LineGraphVis data={data} params={visualParams} />
                )}
                {chartType === 'pie' && (
                  <PieChartVis data={data} params={visualParams} />
                )}
              </motion.div>
            </AnimatePresence>
          </div>
