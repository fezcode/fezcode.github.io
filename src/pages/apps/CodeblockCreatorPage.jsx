...
const CodeblockCreatorPage = () => {
  ...
  const renderCanvas = useCallback(() => {
    ...
    const contentWidth = Math.max(
      600 * fontScale,
      metrics.width + gutterWidth + paddingX * fontScale * 2,
    );
    const contentHeight =
      metrics.height + paddingY * fontScale * 2 + 40 * fontScale; // + header height
    ...
    if (showLineNumbers) {
      ...
      let numY = startY;
      const numX =
        windowX + paddingX * fontScale + gutterWidth - 20 * fontScale; // Right align with some padding
  ...
  const glassInputClass =
    'w-full bg-black/40 border border-white/10 rounded-sm p-3 font-mono text-sm text-gray-300 focus:border-emerald-500/50 outline-none transition-colors';
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
            <div className="space-y-4">
              ...
              <p className="text-xl text-gray-400 max-w-2xl font-light leading-relaxed">
                Render syntax-highlighted code into high-fidelity canvas
                artifacts.
              </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <CustomDropdown
                    ...
                    onChange={(val) =>
                      setBackground(backgrounds.find((b) => b.name === val))
                    }
                    variant="brutalist"
                  />
                  <button
                    onClick={() => setShowBackground(!showBackground)}
                    className={`w-full mt-2 py-2 px-3 rounded-sm border transition-all font-mono text-[10px] uppercase tracking-widest flex items-center justify-between ${
                      showBackground
            ...
            <div className="border border-white/10 bg-white/[0.02] p-6 space-y-6 rounded-sm">
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-sm border transition-all font-mono text-[10px] uppercase tracking-widest flex items-center justify-between ${
                    showLineNumbers
                      ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400'
                      : 'bg-black/20 border-white/10 text-gray-500'
                  }`}
