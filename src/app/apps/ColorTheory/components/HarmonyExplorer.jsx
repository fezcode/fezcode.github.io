      ...
      <div className="w-full max-w-5xl space-y-12">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start justify-between bg-white p-8 rounded-3xl border-2 border-[#1a1a1a] shadow-[8px_8px_0px_0px_#1a1a1a]">
          <div className="flex-1 w-full">
            <label className="block text-sm font-black text-[#1a1a1a] uppercase tracking-wider mb-4">
              Base Hue:{' '}
              <span className="bg-[#1a1a1a] text-white px-2 py-1 rounded-md">
                {baseHue}°
              </span>
            </label>
            <input
                ...
                return (
                  ...
                  >
                    <div className="absolute inset-0 bg-white/90 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <span className="text-[#1a1a1a] font-mono font-bold border-2 border-[#1a1a1a] bg-white px-3 py-1 rounded-md shadow-[2px_2px_0px_0px_#1a1a1a]">
                        {hex.toUpperCase()}
                      </span>
                    </div>
                  </motion.div>
        ...
        <div className="p-8 rounded-3xl bg-white border-2 border-[#1a1a1a] shadow-[8px_8px_0px_0px_#1a1a1a]">
          <h3 className="text-2xl font-normal mb-4 text-[#1a1a1a] font-instr-serif">
            How it works
          </h3>
          <p className="text-[#333] text-lg leading-relaxed font-medium">
            {harmonyType === 'complementary' &&
              'Complementary colors are opposite each other on the color wheel. They create the strongest contrast and visual tension.'}
            {harmonyType === 'analogous' &&
              'Analogous colors are next to each other on the wheel. They usually match well and create serene and comfortable designs.'}
            {harmonyType === 'triadic' &&
              'Triadic color schemes use colors that are evenly spaced around the color wheel. They tend to be quite vibrant, even if you use pale or unsaturated versions of your hues.'}
            {harmonyType === 'split-complementary' &&
              'The split-complementary color scheme is a variation of the complementary color scheme. In addition to the base color, it uses the two colors adjacent to its complement.'}
            {harmonyType === 'tetradic' &&
              'The tetradic (rectangle) color scheme uses four colors arranged into two complementary pairs. This rich color scheme offers plenty of possibilities for variation.'}
            {harmonyType === 'monochromatic' &&
