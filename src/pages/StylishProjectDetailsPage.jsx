...
const StylishProjectDetailsPage = () => {
  ...
  const projectMetadata = projects.find(p => p.slug === slug);
      ...
      const fetchContent = async () => {
        try {
          const productSlug = slug || 'fezcodex';
                  const [heroRes, featuresRes, partnersRes, terminalRes, detailsRes, technicalRes, integrationsRes, ctaRes] = await Promise.all([
          ...
          const [hero, features, partners, terminal, details, technical, integrations, cta] = await Promise.all([
            heroRes.text(),
            featuresRes.text(),
            partnersRes.text(),
            terminalRes.text(),
          ...
          setContent({ hero, features, partners, terminal, details, technical, integrations, cta });
        } catch (error) {
    ...
    if (loading) return <div className="min-h-screen bg-product-bg flex items-center justify-center text-white font-nunito">Loading...</div>;
      ...
      let heroImage = '';
      let heroDescription = '';
      const imageLineIndex = heroLines.findIndex(line => line.startsWith('image:'));
      if (imageLineIndex !== -1) {
      ...
      const partnersLines = content.partners && !content.partners.includes('<!DOCTYPE html>') ? content.partners.split('\n') : [];
      ...
      if (partnersLines.length > 0) {
        ...
        if (logosLine) {
          partners = logosLine.replace('logos:', '').split(',').map(p => p.trim()).filter(Boolean);
        } else {
          partners = partnersLines.filter(l => !l.startsWith('label:')).join(',').split(',').map(p => p.trim()).filter(Boolean);
        }
      }
      ...
      return (
        <div className="min-h-screen bg-product-bg text-product-body-text font-nunito selection:bg-product-card-icon/30 selection:text-white">
          ...
          <main className="max-w-7xl mx-auto px-6 pb-24">
            ...
            <section className="pt-20 pb-8">
              ...
              <div className="max-w-3xl mx-auto text-center mt-4">
                <ReactMarkdown
                  ...
                  components={{
                    p: ({children}) => <p className="text-xl md:text-2xl text-product-body-text leading-relaxed font-nunito">{children}</p>
                  }}
                >
                  {heroDescription}
                </ReactMarkdown>
                  ...
                  {integrations.map((item, idx) => (
                    <div key={idx} className="flex flex-col space-y-6 group">
                      ...
                      <div className="space-y-4">
                        <h3 className="text-2xl md:text-3xl font-instr-serif text-white italic">{item.title}</h3>
                        <p className="text-base text-product-body-text leading-relaxed">{item.description}</p>
                        {item.link && (
                          <Link
                            to={item.link}
            ...
            {techItems.length > 0 && (
              <section id="technical" className="py-24 border-t border-white/5">
                <h2 className="text-4xl md:text-6xl font-instr-serif text-white italic mb-16 text-center">Technical Overview</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {techItems.map((item, idx) => (
                    <div key={idx} className="bg-white/[0.03] p-8 rounded-2xl border border-white/5 hover:bg-white/[0.05] transition-all hover:border-product-card-icon/20 group">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm]}
                        components={{
                          p: ({children}) => <p className="text-product-body-text leading-relaxed font-nunito">{children}</p>,
                          strong: ({children}) => <span className="text-white block mb-2 text-lg font-instr-serif italic">{children}</span>
                        }}
                      >
                        {item._raw}
                      </ReactMarkdown>
              ...
              <section id="details" className="py-24 border-t border-white/5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                  <ReactMarkdown
                    ...
                    rehypePlugins={[rehypeRaw]}
                    components={{
                      h2: ({children}) => <h2 className="text-3xl md:text-5xl font-instr-serif text-white italic mb-6 mt-12 first:mt-0">{children}</h2>,
                      p: ({children}) => {
                        if (typeof children === 'string' && children.startsWith('image:')) {
                          ...
                          return (
                            <div className="my-12 rounded-xl overflow-hidden border border-white/10 hover:border-product-card-icon hover:shadow-[0_0_30px_rgba(217,119,87,0.15)] transition-all duration-500 shadow-xl">
                              <img src={src} alt="Detail Preview" className="w-full h-auto" />
                            </div>
                          );
                        }
                        return <p className="text-lg text-product-body-text leading-relaxed mb-6">{children}</p>;
                      },
                      a: ({href, children}) => {
                        const isInternal = href.startsWith('/');
                        if (isInternal) {
                          return (
                            <Link to={href} className="inline-block bg-white/5 border border-white/10 px-6 py-3 rounded-lg text-white font-bold hover:bg-white/10 transition-all hover:scale-105 active:scale-95">
                              {children}
                            </Link>
                          );
                        }
                        return (
                          <a href={href} target="_blank" rel="noopener noreferrer" className="inline-block bg-white/5 border border-white/10 px-6 py-3 rounded-lg text-white font-bold hover:bg-white/10 transition-all hover:scale-105 active:scale-95">
                            {children}
                          </a>
                        );
                      }
            ...
            {content.cta && !content.cta.includes('<!DOCTYPE html>') && (
              <section id="cta" className="py-24 border-t border-white/5 text-center">
                <ReactMarkdown
                  remarkPlugins={[remarkGfm]}
                  components={{
                    h1: ({children}) => <h2 className="text-4xl md:text-6xl font-instr-serif text-white italic mb-8">{children}</h2>,
                    p: ({children}) => <p className="text-lg text-product-body-text mb-8">{children}</p>,
                    code: ({children, inline}) => {
                      if (inline) return <code className="bg-white/10 px-1.5 py-0.5 rounded font-mono text-product-card-icon">{children}</code>;
          ...
          <footer className="py-12 border-t border-white/5 text-center text-lg font-instr-serif text-product-body-text/50">
            &copy; {new Date().getFullYear()} Handcrafted by Fezcode &nbsp; &middot; &nbsp; Built with love and code.
          </footer>
        </div>
      );
    };
