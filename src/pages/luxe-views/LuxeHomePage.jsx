...
const StatusItem = ({ label, value, icon: Icon, onClick, actionLabel }) => {
  ...
  return (
    <Component
      onClick={onClick}
      whileHover={
        isClickable ? { backgroundColor: 'rgba(255, 255, 255, 0.6)' } : {}
      }
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span className="font-outfit text-[10px] uppercase tracking-[0.2em] text-black/40">
          {label}
        </span>
      </div>
...
const ProjectCard = ({ project, index, isStacked }) => {
  ...
  const innerClass =
    'relative aspect-[3/4] md:aspect-square lg:aspect-[4/5] w-full bg-[#EBEBEB] rounded-xl overflow-hidden border border-white/20 group ' +
      ...
      <div className={innerClass}>
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-black/5 p-8 flex flex-col justify-between text-white">
            ...
            <div>
              <h3
                className={`font-playfairDisplay italic mb-4 ${isStacked ? 'text-4xl md:text-6xl' : 'text-3xl md:text-4xl'}`}
              >
                {project.title}
              </h3>
              <p
                className={`font-outfit text-white/80 leading-relaxed max-w-md ${isStacked ? 'text-sm md:text-base line-clamp-4' : 'text-xs md:text-sm line-clamp-3'}`}
              >
...
const JournalCard = ({ post, index, isStacked }) => {
  ...
  const innerClass =
    'relative aspect-[3/4] md:aspect-square lg:aspect-[4/5] w-full bg-white rounded-xl overflow-hidden border border-black/5 flex flex-col group ' +
    (isStacked
      ? 'shadow-2xl max-h-[700px]'
      ...
      <Link to={`/blog/${post.slug}`} className="block h-full">
        <div className={innerClass}>
          ...
          <div className="flex-1 p-8 flex flex-col justify-center items-center text-center bg-[#FDFCFB] group-hover:bg-[#FFF] transition-colors relative overflow-hidden">
            ...
            />
            <h3
              className={`font-playfairDisplay text-[#1A1A1A] leading-tight group-hover:scale-105 transition-transform duration-700 ease-out ${isStacked ? 'text-4xl md:text-6xl' : 'text-3xl md:text-4xl lg:text-5xl'}`}
            >
              {post.title}
            </h3>
          </div>
          <div className="p-6 border-t border-black/5 flex justify-between items-center bg-[#FAFAF8]">
            ...
            <ArrowRightIcon
              size={20}
              className="text-black/30 group-hover:text-[#8D4004] group-hover:translate-x-2 transition-all"
            />
          </div>
        </div>
      </Link>
      ...
      <div className="border-y border-black/10 bg-white/40 backdrop-blur-md sticky top-0 z-40">
        <div className="flex justify-center divide-x divide-black/10 max-w-[1800px] mx-auto overflow-x-auto no-scrollbar">
          <StatusItem
            ...
            onClick={() =>
              setLayoutMode((prev) => (prev === 'stack' ? 'grid' : 'stack'))
            }
            actionLabel="Toggle"
          />
          <StatusItem
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          <div className="order-1 lg:order-1">
            <div className="mb-16 border-b border-black/10 pb-6">
              <h2 className="font-playfairDisplay text-4xl text-[#1A1A1A]">
                Latest{' '}
                <span className="italic text-black/50">Observations</span>
              </h2>
            </div>
            {posts.slice(0, 5).map((post, i) => (
              <JournalCard
                ...
                post={post}
                index={i}
                isStacked={layoutMode === 'stack'}
              />
            ))}
            <div className="mt-12 text-center lg:text-left">
              <Link
                to="/blog"
                className="font-outfit text-xs uppercase tracking-widest border-b border-black pb-1 hover:text-black/60 transition-colors"
              >
          ...
          <div className="order-2 lg:order-2">
            <div className="mb-16 border-b border-black/10 pb-6 text-right">
              <h2 className="font-playfairDisplay text-4xl text-[#1A1A1A]">
                Selected <span className="italic text-black/50">Works</span>
              </h2>
            </div>
            {loadingProjects ? (
              <div className="py-20 text-center font-outfit opacity-50">
                Loading Collections...
              </div>
            ) : (
              projects
